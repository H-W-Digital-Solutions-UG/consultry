...
### Hero
- Grundzustand (ohne JS, Reduced Motion, Data-Saver, 2g): Vollhöhe (`100svh`), Bild `object-fit: cover`, Schleier (siehe 3), Inhalt unten links. Eyebrow → `h1` (`--fg-on-ink`) → Claim (Achsen-Verlauf als Textfüllung) → Beschreibung (`--fg-on-ink-muted`) → Buttons. Rechts daneben (ab 900 px, um 48 px angehoben, damit das Förderband im Motiv frei bleibt) bzw. darunter die drei Beats als einfache Liste (`ol` mit `h3`/`p`, Daten `heroBeats` in `content.ts`). Die Liste trägt die Ziffern 01–03; es gibt **keine** Annotationen auf dem Bild.
- Flache Viewports (≤ 700 px hoch, z. B. 375×667 oder 844×390 quer): `padding-top` der Copy = Nav-Höhe + 24 px, `h1` `clamp(1.75rem, 3.8vw, 2.5rem)`, Beschreibung 0,9 rem, kompakte Buttons – die Copy darf nie beschnitten werden.
- Scroll-Szene (Progressive Enhancement, `src/scripts/hero-scroll.ts`): Track 220 vh (mobil 200 vh), Bühne `position: sticky`, `height: auto; min-height: 100dvh` (Fallback `100svh`); `overflow: hidden` liegt nur auf dem Bildlayer. Der Bildlayer ist `max(178svh, 150vw)` breit und fährt linear mit dem Scroll von Bildlinks (Prozess-Knoten) nach Bildrechts (Produktion), dazu Skalierung 1,06 → 1,0 und 1,5 % vertikale Drift – eine einzige Kamerafahrt, kein Nachschwingen. Beat-Index bei 10 / 40 / 70 %. Desktop: die ganze Beat-Liste ist ab `--p` 0 mit Opacity .45 sichtbar (Scroll-Hinweis), der aktive Beat wird auf 1 gehoben und erhält die ruhige Fläche `rgba(245,241,251,.06)` mit Rahmen `--line-on-ink` statt Glow, bereits passierte Beats bleiben bei .8 (240 ms). Copy tritt ab carry .46 (≈ 73 % der Strecke) zurück: Eyebrow und Beschreibung Opacity .5, `h1` .7 mit −10 px und `scale(.98)`, Claim .6 – **die CTAs bleiben immer bei voller Deckkraft und bedienbar**; der Ruhezustand am Ende muss aufgelöst wirken, nicht wie ein Platzhalter.
- Mobil (< 900 px): sobald Beat 1 aktiv ist, verlässt der Copy-Block die Bühne nach oben (`translateY(-100dvh)`, Opacity 0, 300 ms, `visibility: hidden`), Bild und Beats besitzen das Bild. Die drei Beats stehen gestapelt unten (gleiche Aktiv-/Passiert-Logik, kein Kartenwechsel), darunter ein kompakter Primär-CTA „Erstgespräch vereinbaren“ (`.btn.btn--primary.hero__side-cta`, kleinerer Innenabstand). Quer (≤ 500 px hoch) stehen die Beats nebeneinander. Der statische Zustand (kein JS, Reduced Motion) zeigt weiterhin die volle Copy.
- Fortschrittslinie 2 px in `--axis` am unteren Rand der Bühne; eine Aubergine-Abdeckung schiebt sich mit `--p` weg, sodass Magenta nur am Ende erscheint.
- Farbwash: zwei weiche lineare Verläufe, per `--p` gegeneinander geblendet – `--process` maximal 6 % im linken 40 %-Band, `--production` maximal 5 % im rechten 35 %-Band (z. B. `linear-gradient(255deg, color-mix(in oklab, var(--production) 5%, transparent) 0%, transparent 40%)`). Magenta bleibt Akzent, nie Fläche. Dazu ein statischer 111°-Schleier aus `--ink` (~12 %) über dem mittleren Band, der das Einlauf-Leuchten des Motivs zähmt. Kein Bloom, kein Grain, keine Partikel, keine radialen Verläufe – das Leuchten stammt allein aus dem Bildmotiv.
