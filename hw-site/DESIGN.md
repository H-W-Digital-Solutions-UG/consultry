...
### Hero
- Statisch (seit 2026-09-18, Gründer: kein Hintergrund-Scroll): Vollhöhe (`100svh`, Raster `1fr auto`), Bild als Fläche hinter Copy und Nachlauf, `object-fit: cover`, `object-position: 68% 45%`, entsättigt und abgedunkelt (`saturate(.72) brightness(.78)`). Kein Pan, keine Skalierung, keine Fortschrittslinie, kein Farbwash.
- Schleier: Vignette `radial-gradient(120% 90% at 30% 42%)` von 0 auf 86 % `--ink` (Licht bleibt um die Copy), dazu ein Verlauf nach unten in `--ink` (100 % ab Unterkante, 0 ab 55 %), ein Lesbarkeits-Verlauf von links (72 % → 0 bei 66 %) und ein leichter Verlauf von oben (55 % → 0 bei 30 %). Keine Leuchtflecken, kein Glow.
...
- Mobil (< 900 px): `object-position: 72% center`, dichterer Schleier nach unten; die Leiste folgt der Copy.
