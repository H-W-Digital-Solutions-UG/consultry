...
### Hero
- Statisch (seit 2026-09-18, Gründer: kein Hintergrund-Scroll): Vollhöhe (`100svh`, Raster `1fr auto`), Bild als Fläche hinter Copy und Nachlauf, `object-fit: cover`, `object-position: 30% 42%`, kaum gedämpft (`saturate(.92) brightness(1)`). Kein Pan, keine Skalierung, keine Fortschrittslinie, kein Farbwash.
- Bild (`src/assets/hero.webp`, 2560 × 1440, generiert mit Higgsfield, Modell GPT Image 2.5 in hoher Qualität, 4K-Upscale, Referenz: Gründer-Video vom 2026-09-18): ein abstrakter, analog wirkender Lichtverlauf mit feinem Filmkorn. Seit der zweiten Runde (2026-09-18, Gründer: „hochprofessionell, minimalistisch, aber varied“) ein ruhiges Feld in `--ink`, in der rechten Hälfte eine einzige sehr große, weich gerandete Scheibe in staubigem Mauve, völlig unscharf, deren Kante von rechts oben nach unten zur Mitte läuft; die linke Hälfte bleibt gleichmäßig und leicht aufgehellt, damit die Copy dort trägt. Keine zweite Form, keine Linien, kein Bokeh, keine Leuchtflecken. Das Bild gibt der leeren rechten Hälfte eine Form und lässt die Copy auf der ruhigen Seite. Verworfen in dieser Runde: gleichmäßiges Oberlicht (flach, nimmt der Headline Kontrast), zwei überlagerte Papierflächen (die Kante läuft durch die Copy). Der Lichtpool aus Runde eins bleibt als Alternative dokumentiert (`.agents/product-marketing.md`).
- Schleier: nur noch drei leichte Verläufe – nach unten in `--ink` (100 % ab Unterkante, 55 % bei 16 %, 0 ab 40 %) für den nahtlosen Übergang zur Trust-Leiste, ein Hauch von links (14 % → 0 bei 40 %) und ein Hauch von oben (30 % → 0 bei 20 %). Keine radiale Vignette mehr, keine Leuchtflecken, kein Glow.
...
| Datum | Änderung |
|---|---|
| 2026-09-18 | Hero-Hintergrund, zweite Runde: große weiche Scheibe rechts statt Lichtpool links (Gründer: „hochprofessionell, minimalistisch, aber varied“); drei Konzepte im Layout verglichen (Oberlicht, Bogen, Papierflächen). |
| 2026-09-18 | Kanonische Adresse auf `consultry.de` umgestellt (Gründer: die Seite zieht auf consultry.de, die Produktseite auf consultry.app). Betroffen: `site` in der Astro-Konfiguration, `domain` und `siteUrl` in den Firmendaten (speisen strukturierte Daten), Sitemap-Verweis in der robots.txt. Die Kontaktadresse bleibt `contact@hw-digitalsolutions.de`: dort liegen die Microsoft-365-Postfächer, die Domain darf deshalb ihre MX- und TXT-Einträge behalten. |
| 2026-09-18 | Zweite Ladezeit-Runde, gemessen mit Kompression wie auf dem Server (Lighthouse 13, gzip): mobiler LCP 2,3 → 2,0 s, real gemessen 331 → 150 ms; Seitengewicht 223 → 185 KB; Desktop 100 mit LCP 0,4 s. (1) Das Einblenden läuft ohne Deckkraft: neue Kurve `settle-up` bewegt nur noch `translateY`, der Text steht von der ersten Bildzeile an. Chrome zählt ein Element mit `opacity: 0` nicht als gezeichnet, deshalb schob die alte Blende den LCP-Zeitpunkt um die volle Verzögerung. (2) Schriften mit `scripts/fonts.py` zugeschnitten: Gewichtsachse auf Sora 300–700 und Inter 300–600, Zeichenvorrat auf Basis-Latein, Latin-1 und die genutzten Satzzeichen; 80 → 54 KB. Die `unicode-range` in `global.css` beschreibt genau diesen Vorrat, damit ein fremdes Zeichen auf die Systemschrift fällt statt leer zu bleiben. Beide Stellen gehören zusammen. (3) Partnerlogo-Masken von 96 auf 72 px Höhe: 53 → 41 KB. Die Leiste steht auf beiden Größen im ersten Bild, Nachladen wäre also falsch. |
| 2026-09-18 | Ladezeit-Runde (Lighthouse 13, lokaler Build): mobil 92 → 97, LCP 3,3 → 2,6 s, Seitengewicht 450 → 304 KB; Desktop bleibt 100, LCP 0,7 → 0,5 s. Vier Eingriffe: (1) Einblenden im Hero und Seitenkopf gestrafft (Versatz 0,04–0,28 s statt 0,1–0,7 s, Dauer 0,55 s statt 0,8 s) — der Hero-Text ist mobil das LCP-Element, seine Verzögerung zählt unter Drosselung vierfach; (2) Partnerlogos laufen als CSS-Maske und werden als WebP mit 96 px Höhe vorgerendert (Qualität 20, da die Maske die Farbkanäle verwirft): 203 → 53 KB; (3) Proof-Szene zusätzlich in 480 px mit engerem `sizes`; (4) Favicon-Dateien mit Cache-Lebensdauer von einer Woche in `vercel.json`. Nicht angefasst: die eingebettete CSS (komprimiert wenige KB) und die Schriften. |
| 2026-09-18 | Favicon: die H&W-Marke (H-Kasten, &, W-Kasten, ohne Wortmarke) weiß auf `--ink` im abgerundeten Quadrat, als ICO 16/32/48, PNG 512 und Apple-Touch-Icon 180; das generische Verlaufsquadrat (`favicon.svg`) entfällt. Quelle: H&W-Logo aus der Angebotsvorlage, in `src/assets/` abgelegt. |
| 2026-09-18 | Proof-Szene: Headline „Der Senior im Angebot ist der Senior im Projekt.“ mit Text zu bezahlter Erfahrung und den eigenen Nachweisen der Gründer; Antwort auf Body Leasing (Gründer). Zwischenstände „Geplant und gebaut für genau einen Betrieb: Ihren.“ und „Sie erklären Ihren Betrieb genau einmal.“ verworfen. |
| 2026-09-18 | Einwand-Visuals genauer auf den Hook abgestimmt (Gründer: „Richtung gut, akkurater abstimmen“). 1: Einzelne fragen in alle Richtungen, Ring „?“ als Ergebnis offen, Pfeil, ein Ablauf mit allen Punkten, Ring mit Haken als Ergebnis geprüft. 2: Kurven jetzt symmetrisch, Kreuzung als Ring „Übergabe“ markiert, Labels „Wir übernehmen“ / „Ihr Team übernimmt“. 3: Balken „Nutzen je Ablauf, vorher gerechnet“, gestrichelte Linie „Kosten“, oberhalb „rechnet sich“, unterhalb „rechnet sich nicht“; sechs Balken, damit das Label frei steht. 4: Pilot endet an einem Ring „Pilot, endet hier“, die Lösung läuft in ein Systemfenster mit Titelleiste, Label „In Ihren Systemen, täglich“. Art `customer` aus `PromiseViz` entfernt (ohne Verwendung seit der Bildkarte). |
