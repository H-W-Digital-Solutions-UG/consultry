# Consultry Design System v1.3

## AI-Native Consultancy CRM for the DACH Market

**Status:** Revised — 2026 Design Philosophy Integration
**Date:** 31 March 2026
**Product:** Consultry
**Audience:** Design, Engineering, Product
**Changelog:** v1.3 integrates 2026 design philosophy: voice input pattern (Command Bar + Copilot), adaptive bento grid system, mobile bottom navigation bar, kinetic typography for AI reveals, preview panel pattern for documents/files, professional emoji guidelines, detailed illustration style guide, AI-adaptive layout rules, predictive intelligence UI patterns. Confirms light theme as default (dark = toggle). Stitch/amber tokens deprecated — DS v1.3 is the sole canonical source. See Section 10 for full version history.
...
### 1.10 Professionelle Emoji-Richtlinien (Professional Emoji Guidelines)

Consultry uses emoji sparingly to add warmth and scannability — never as decoration or personality filler. Emoji are a visual shorthand, not a tone.

**Erlaubt (Allowed Contexts):**

| Context | Beispiel | Regel |
|---------|----------|-------|
| Status-Badges & Tags | `✅ Eingewilligt`, `⚠️ Ausstehend` | Emoji als visueller Marker neben Text-Label. Nie alleinstehend. |
| Empty States | `📭 Noch keine Signale` | Ein Emoji pro Empty-State-Titel. Verstärkt die Botschaft. |
| Notifications & Toasts | `🎉 Deal gewonnen!`, `📋 Neue Ausschreibung` | Vorangestelltes Emoji für schnelles Scanning in der Notification-Inbox. |
| Signal-Feed-Kategorien | `⚡ Leadership-Wechsel`, `📋 Ausschreibung` | Kategorie-Indikator im Signal-Feed. Konsistent pro Signal-Typ. |
| KPI-Trends | `📈 +12%`, `📉 -3%` | Neben Trend-Werten in Dashboards. Optional. |

**Verboten (Forbidden Contexts):**

- **Fließtext / Body Copy** — Keine Emoji in Absätzen, Beschreibungen oder AI-generierten Briefings.
- **Button-Labels** — Nie: `🚀 Absenden`. Immer: `Absenden`.
- **Form Labels & Platzhalter** — Keine Emoji in Eingabefeldern.
- **Überschriften / Headings** — Ausnahme: Empty-State-Titel (siehe oben).
- **Tabellenzellen** — Keine Emoji als Datenwerte. Status-Badges verwenden stattdessen die Badge-Komponente.
- **Navigationselemente** — Sidebar, Topbar, Tabs: nur Lucide-Icons, nie Emoji.

**Regel:** Im Zweifel: kein Emoji verwenden. Wenn ein Lucide-Icon denselben Zweck erfüllt, ist das Icon vorzuziehen. Emoji sind ein Garnish, kein Hauptgericht.
...
**Farbsystem-Klärung (v1.3):**

- **Light Theme ist Standard.** Alle Specs, Screens und Komponenten werden zuerst im Light Theme spezifiziert.
- **Dark Theme ist ein Toggle** — verfügbar als Nutzerpräferenz, nicht als separates Design. Aktivierung via Token-Remapping (siehe Abschnitt 9).
- **Stitch/Amber-Tokens sind veraltet.** Das Stitch Inspiration Board ("Consultry-Inspiration") verwendete ein Dark/Amber-Theme als kreative Inspiration. Diese Tokens sind NICHT kanonisch und dürfen nicht in Produkt-Specs referenziert werden.
- **Journey-Inline-Tokens** aus den User-Journey-Dateien (z.B. `--warm-coral`, `--sidebar-dark`) werden auf die kanonischen DS-v1.3-Tokens gemappt. Keine eigenen Token-Definitionen außerhalb dieses Design Systems.
...
#### 3.6.1 Kinetische Typografie (Kinetic Typography)

Kinetic typography adds life to AI-generated content and data displays without crossing into animation excess. All kinetic effects are opt-in — they enhance understanding, never distract from it.

| Token | Value | Usage |
|-------|-------|-------|
| `ktype-ai-reveal` | Per-character: 12ms delay, opacity 0→1, translateY(4px→0). Max 300ms total per line. | AI "typewriter" reveal — Copilot and Knowledge Agent responses appear character-by-character. Triggered only on initial generation, not on re-reads. |
| `ktype-score-counter` | Number rolls from 0→value, `duration-slow` (400ms), `easing-enter`. | Match scores, KPI counters, financial figures when first entering viewport. Uses `mono-md`. |
| `ktype-heading-emphasis` | Word-by-word fade-in, 60ms stagger, opacity 0→1. | Onboarding headings, empty-state titles, and login hero text. Uses `display-hero` or `display-lg`. |
| `ktype-label-shift` | Cross-fade between text values, `duration-normal` (250ms). | Dynamic labels that update (e.g., filter count, live score). Smooth value transition instead of instant swap. |

**Kinetic Typography Rules:**

- **Einmalig (One-shot):** AI reveal and heading emphasis play once on first render. No loops, no replay on scroll-back.
- **Deaktivierbar:** Respect `prefers-reduced-motion: reduce` — all kinetic effects degrade to instant render.
- **Keine Ablenkung:** Kinetic effects run at most on one element at a time per viewport. If multiple elements enter simultaneously, stagger entry by 150ms.
- **Kein Bouncing, kein Spinning:** Only opacity + translateY transitions. No scale, no rotation, no elastic easing.
...
### 4.1 Detaillierte Illustrationen (Detailed Illustrations)

Consultry uses detailed, purpose-built illustrations for specific UI contexts where icons alone are insufficient. These illustrations add warmth, professionalism, and visual storytelling.

**Stil-Richtlinien:**

| Eigenschaft | Regel |
|-------------|-------|
| **Linienstärke** | 1.5–2px Stroke. Konsistent mit Lucide-Icon-Grammatik, aber in größerem Maßstab (120–240px). |
| **Farbpalette** | Warm palette only: `neutral-700`, `neutral-400`, `brand-primary`, `brand-warm`. Maximal 3 Farben pro Illustration. |
| **Stil** | Line-art mit selektiver Flächenfüllung. Keine fotorealistischen Elemente. Keine 3D-Effekte. |
| **Stimmung** | Professionell, ruhig, unterstützend. Keine Cartoons, keine anthropomorphisierten Objekte. |
| **Hintergrund** | Transparent — Illustrationen stehen auf der jeweiligen Surface (`neutral-0`, `neutral-100`). |
| **Größen** | `illust-sm` (120px), `illust-md` (180px), `illust-lg` (240px). Zentriert im Container. |

**Einsatzorte:**

| Kontext | Illustration | Beschreibung |
|---------|-------------|-------------|
| **Empty States** | Themenspezifisch | Leere Pipeline: stilisierte Trichter-Line-Art. Kein Berater gefunden: Profil-Outline mit Suchsymbol. |
| **Onboarding** | Workflow-Schritte | Vereinfachte Darstellung der Plattform-Bereiche. Pro Schritt eine Illustration. |
| **Fehler-Seiten** | Kontextbezogen | 404: Kompass mit gebrochenem Pfeil. 500: Zahnrad mit Ausrufezeichen. Ruhig, nicht frustrierend. |
| **Lade-Zustände** | Platzhalter | Nur bei Full-Page-Loads: Logo-Mark mit sanfter Pulse-Animation (siehe 3.6). Nie als Inline-Shimmer-Ersatz. |
| **Erfolgs-Bestätigungen** | Feier-Moment | Deal gewonnen: stilisierter Handschlag. Onboarding abgeschlossen: Checkmark mit Konfetti-Linien. Dezent. |

**Regel:** Illustrationen ergänzen Text, ersetzen ihn nie. Jede Illustration braucht einen begleitenden Titel (`heading-md`) und eine Beschreibung (`body-md`). Nie eine Illustration ohne Kontext-Text zeigen.
...
### 5.12 Bottom Navigation Bar (Mobile)

Die Bottom Navigation Bar ersetzt Hamburger-Menüs auf mobilen Geräten. Sie bietet konstante, daumenfreundliche Navigation zu den wichtigsten Bereichen.

| Property | Value |
|----------|-------|
| Höhe | 56px + Safe-Area-Inset (iOS) / 56px (Android) |
| Hintergrund | `neutral-0` mit `border-subtle` top-border |
| Items | 4–5 max. Mehr als 5 Items erfordern ein "Mehr"-Overflow-Menü. |
| Icon-Größe | `icon-md` (20px) |
| Label | `body-xs` (11px), `neutral-600` (inaktiv), `brand-primary` (aktiv) |
| Aktiv-Indikator | Filled icon + `brand-primary` Farbe + optional: Pill-Hintergrund `interactive-warm` (24×4px, `radius-full`, zentriert unter Icon) |
| Inaktiv | Outline icon, `neutral-500` |
| Badge | `semantic-danger` Dot (8px, `radius-full`) oben-rechts am Icon. Zahlen-Badge ab 2+ Notifications. |
| Safe-Area | `env(safe-area-inset-bottom)` — respektiert iOS Home-Indikator. |
| Haptik | Leichte Haptik-Feedback auf Tap (`UIImpactFeedbackGenerator.style.light` auf iOS). |

**Standard-Items (Persona-adaptiv):**

| Persona | Item 1 | Item 2 | Item 3 | Item 4 | Item 5 |
|---------|--------|--------|--------|--------|--------|
| Katrin (BD) | Signale | Pipeline | Matching | Copilot | Mehr |
| Thomas (MP) | Cockpit | Approvals | Pipeline | Berichte | Mehr |
| Stefan (SC) | Projekte | Wissen | Profil | Team | — |
| Lisa (C) | Profil | Aufgaben | Wissen | — | — |

**Regeln:**

- Nur auf `breakpoint-sm` (< 640px) und `breakpoint-md` (< 768px) sichtbar. Ab `breakpoint-lg` wird die Desktop-Sidebar verwendet.
- Nie gleichzeitig mit der Desktop-Sidebar anzeigen.
- Die Bottom Navigation Bar liegt immer ÜBER dem Content (fixed position, `z-index: 50`).
- Scrollbare Inhalte erhalten `padding-bottom: 72px` (56px Bar + 16px Breathing Room) um verdeckte Inhalte zu vermeiden.
...
### 6.9 Voice Input Pattern

Consultry unterstützt Spracheingabe als ergänzenden Input-Modus in ausgewählten Oberflächen. Voice ist immer additiv — nie der einzige Eingabeweg.

**Platzierung:**

| Oberfläche | Position | Verhalten |
|-----------|----------|-----------|
| Command Palette (5.10) | Rechts im Input-Feld, neben Submit | Öffnet Voice-Modus innerhalb der Palette. |
| Copilot Panel | Rechts im Prompt-Feld | Voice-Input direkt in den Copilot-Chat. |
| Globale Suche (Top Bar) | Rechts im Suchfeld | Sprache wird zu Text transkribiert, Suche startet. |
| Knowledge Agent Chat | Rechts im Chat-Input | Frage wird gesprochen, transkribiert, gesendet. |

**Mikrofon-Button:**

```
┌────────────────────────────────────────────────────┐
│  [🔍] Suchen oder Aktion starten...          [🎤]  │  ← Idle
│  [🔍] Suchen oder Aktion starten...    [🎤 ●●●]   │  ← Listening (pulsing)
│  [🔍] "SAP Migration Retail..."        [⏳]        │  ← Processing
│  [🔍] SAP Migration Retail                  [🎤]   │  ← Completed (text filled)
└────────────────────────────────────────────────────┘
```

| State | Icon | Visuell | Animation |
|-------|------|---------|-----------|
| **Idle** | `Mic` (Lucide) | `neutral-500`, 20px | — |
| **Hover** | `Mic` | `brand-primary` | `duration-fast` |
| **Listening** | `Mic` + 3 animierte Dots | `semantic-success`, Pulsing Ring `semantic-success-light` | Dots: 150ms stagger, opacity 0.4→1.0 loop. Ring: scale 1→1.3, opacity 0.6→0, 1.2s loop. |
| **Processing** | `Loader2` (spinning) | `brand-primary` | `duration-slow`, rotate 360°. |
| **Error** | `MicOff` | `semantic-danger` | Flash 2×, then zurück zu Idle nach 2s. |
| **Nicht verfügbar** | `MicOff` | `neutral-400`, Tooltip: "Mikrofon nicht verfügbar" | — |

**Regeln:**

- **Nur Input, kein Output.** Consultry spricht nicht zurück. Kein Text-to-Speech.
- **Browser Speech API** (`webkitSpeechRecognition` / `SpeechRecognition`). Fallback: Button deaktiviert mit Tooltip.
- **Sprachen:** `de-DE` (Standard), `de-AT`, `de-CH`, `en-US`. Erkennung folgt der UI-Sprache.
- **Transkription sichtbar:** Während des Sprechens wird der transkribierte Text live im Input-Feld angezeigt.
- **Abbruch:** Erneuter Klick auf Mikrofon-Button oder `Escape` bricht ab und verwirft den Text.
- **Datenschutz:** Voice-Daten werden nicht gespeichert. Kein Audio-Logging. DSGVO-konform.

---

### 6.10 Preview Panel Pattern

Das Preview Panel ermöglicht die Inline-Vorschau von Dokumenten, Dateien und strukturierten Inhalten — ohne die aktuelle Ansicht zu verlassen.

**Anatomie:**

```
┌─ Preview Panel ──────────────────────────────────────┐
│  [Thumbnail]  Dateiname.pdf              [↗ Öffnen]  │  ← Header
...
│                                                      │
├──────────────────────────────────────────────────────┤
│  [📥 Herunterladen]  [✏️ Bearbeiten]  [📄 Vollbild]  │  ← Actions
└──────────────────────────────────────────────────────┘
```

**Varianten:**

| Variante | Container | Trigger | Verwendung |
|----------|-----------|---------|------------|
| **Inline** | Eingebettet im Card-Body, 200px Höhe | Automatisch bei Anhängen | Angebots-Canvas, Vertrags-Canvas: Dokument-Vorschau im Workflow. |
| **Slide-Over** | Slide-Over Panel (5.11), 640px breit | Klick auf Dokument-Link | Engagement Brief, CV, Knowledge Asset: Detail-Vorschau. |
| **Tooltip** | Floating, 320×200px, `surface-glass` | Hover auf Datei-Link (Desktop) | Schnelle Vorschau in Listen und Tabellen. `duration-normal` delay. |
| **Fullscreen** | Modal, max-width 1200px | Klick auf "Vollbild" im Panel | PDF-Review, Vertrags-Redlining. |

**Unterstützte Dateitypen:**

| Typ | Vorschau-Methode | Interaktion |
|-----|-----------------|-------------|
| PDF | Gerendert (Page-by-Page Scroll) | Seiten-Navigation, Zoom |
| DOCX/PPTX | Thumbnail + erste Seite als Bild | Download-Link, Bearbeiten in externer App |
| Bilder (PNG/JPG/SVG) | Native Render | Zoom, Pan |
| CSV/Excel | Tabellen-Vorschau (erste 10 Zeilen) | Scroll, Sortierung |
| Markdown | Gerendert als HTML | Scroll |
| AI-generierte Inhalte | Live-Render mit `ai-surface` bg | Bearbeiten, Neu generieren |

**Design Tokens:**

- Background: `neutral-0` (inline), `surface-glass` (tooltip)
- Border: `border-default`, `radius-lg`
- Thumbnail: 48×48px, `radius-md`, `neutral-100` fallback-bg
- Shadow: `shadow-sm` (inline), `shadow-md` (tooltip), `shadow-xl` (fullscreen)

---

### 6.11 AI-Adaptive Layout Rules

Consultry passt Layouts basierend auf Persona, Nutzungsfrequenz und prädiktivem Kontext an. Die Adaption ist subtil — sie reorganisiert Prioritäten, ändert aber nicht die grundlegende Seitenstruktur.

**Persona-basierte Defaults:**

| Persona | Layout-Profil | Widget-Priorität | Informationsdichte |
|---------|--------------|-------------------|-------------------|
| Katrin (BD-Leiterin) | Power-User: maximale Dichte, alle Sidebar-Sektionen offen | Pipeline → Signale → Matching → Outreach | Hoch: L0-Glance zeigt 7+ Datenpunkte |
| Thomas (Managing Partner) | Scan-and-Decide: KPI-first, große Cards | Cockpit-KPIs → Approvals → Pipeline-Summary | Mittel: L0-Glance zeigt 5 Datenpunkte (Standard) |
| Stefan (Senior Consultant) | Projekt-fokussiert: Timeline-zentriert | Projekt-Status → Wissens-Feed → Verfügbarkeit | Mittel |
| Lisa (Junior Consultant) | Geführt: Ein-Aufgabe-pro-Screen | Profil-Aufgaben → Benachrichtigungen | Niedrig: progressive Offenlegung, mehr Whitespace |
| Martina (Office-Managerin) | Admin-Desktop: Tabellen-zentriert, breite Layouts | Berater-Liste → CV-Queue → Compliance | Hoch: Bulk-Aktionen sichtbar |
| Dr. Mueller (Kunde) | Minimal: White-Label, maximal 3 Aktionen pro Screen | Dashboard → Dokumente → Feedback | Sehr niedrig: große Typografie, viel Whitespace |

**Frequenz-basierte Widget-Ordnung:**

Widgets auf Dashboard-Screens (Cockpit, Discovery, Practice-Dashboard) werden basierend auf der Nutzungshäufigkeit der letzten 30 Tage sortiert:

1. **Meistgenutztes Widget** → Position oben-links (Desktop) / oben (Mobile)
2. **Zweithäufigstes** → daneben / darunter
3. Widgets mit 0 Interaktionen in 30 Tagen → automatisch in "Mehr anzeigen" collapsed

**Regeln:**

- Die Persona-Defaults gelten beim ersten Login. Frequenz-basierte Anpassung beginnt nach 14 Tagen Nutzung.
- Nutzer können die automatische Sortierung jederzeit manuell überschreiben ("Widget-Reihenfolge fixieren" in Einstellungen).
- Adaptive Layouts ändern nur die Reihenfolge und Sichtbarkeit von Widgets — nie die Seitenstruktur, Navigation oder Farbgebung.
- Jede automatische Änderung ist für den Nutzer nachvollziehbar: "Basierend auf deiner Nutzung umgeordnet" als `body-xs` Hinweis unter dem Dashboard-Titel.

---

### 6.12 Predictive Intelligence UI

Consultry zeigt proaktive, KI-gestützte Vorschläge an, die dem Nutzer Arbeit abnehmen — ohne aufdringlich zu wirken.

**Muster:**

| Pattern | Visuell | Trigger | Beispiel |
|---------|---------|---------|----------|
| **Smart Pre-Fill** | Input-Feld mit `ai-surface` Hintergrund + `body-sm` Hinweis "KI-Vorschlag" | Formular-Öffnung mit Kontext | Angebots-Canvas: Tagessatz vorausgefüllt basierend auf letzten 3 vergleichbaren Deals. |
| **Proaktiver Nudge** | Toast-ähnliche Card am unteren Rand, `ai-surface` bg, `ai-border` left, sanftes slide-up | Muster-Erkennung | "Du hast 3 Signale zu Retail-SAP unbearbeitet. Soll ich einen Engagement-Brief vorbereiten?" |
| **"Du wirst wahrscheinlich..."-Card** | Card im Context Rail, `ai-surface` bg, `heading-sm` Titel | Vorhersage basierend auf Workflow-Position | "Du wirst wahrscheinlich: Stefan für KW 18 anfragen → [Verfügbarkeit prüfen]" |
| **Smart Defaults** | Dropdown/Select vorausgewählt mit KI-empfohlenem Wert + `ai-sparkle` Icon | Formular mit historischen Daten | Branche: "Retail" vorausgewählt weil 80% der letzten Opportunities Retail waren. |
| **Completion Suggestion** | Inline-Suggestion im Textfeld (grau, wie GitHub Copilot) | Tippen im Freitext-Feld | Outreach-Editor: Satzende-Vorschlag basierend auf Kontakt-Kontext. `Tab` zum Annehmen. |

**Design-Regeln:**

- Alle KI-Vorschläge verwenden `ai-surface` Hintergrund — nie auf `neutral-0`. Der Nutzer muss sofort erkennen: "Das kommt von der KI."
- Jeder Vorschlag hat eine Dismiss-Option (X-Button oder "Nicht hilfreich").
- Maximal 2 proaktive Nudges gleichzeitig sichtbar. Dritter wird in die Notification-Inbox verschoben.
- Smart Pre-Fills sind editierbar — der vorausgefüllte Wert verhält sich wie normaler Input. Kein Lock.
- Completion Suggestions verschwinden bei `Escape` und erscheinen erst nach 500ms Tipp-Pause.
- Confidence-Schwelle: Vorschläge unter 70% Confidence werden nicht angezeigt.
...
### 7.1 Adaptives Bento Grid System

Das Bento Grid ist ein flexibles, Kachel-basiertes Layout-System für Dashboards und Cockpit-Screens. Es wird **nicht** als globales Layout-Primitiv verwendet — Workflow-Screens nutzen weiterhin progressive Disclosure und die 2/3+1/3 Asymmetrie.

**Entscheidungsbaum: Bento Grid vs. Progressive Disclosure**

| Screen-Typ | Layout | Begründung |
|-----------|--------|------------|
| Dashboard / Cockpit (Thomas, Katrin) | **Bento Grid** | Mehrere gleichwertige KPI-Blöcke, keine lineare Workflow-Hierarchie. |
| Financial Intelligence Dashboard | **Bento Grid** | Chart-Kacheln, KPI-Cards, Tabellen nebeneinander. |
| Practice-Lead Dashboard (Stefan) | **Bento Grid** | Team-Übersicht, Kapazität, Skill-Matrix als Kacheln. |
| Discovery Dashboard | **Bento Grid** | Signal-Kategorien, Trend-Charts, Activity-Feed als Kacheln. |
| Opportunity Detail, Matching | **Progressive Disclosure** | Linearer Workflow: Kontext → Entscheidung → Aktion. |
| Profil-Editor, Admin-Panel | **Progressive Disclosure** | Formular-basiert, sequentieller Ablauf. |
| Canvas-Screens (Angebot, Vertrag) | **Progressive Disclosure** | Fokus-Modus, ein Dokument + Tools. |
| Mobile Screens | **Stack (Linear)** | Bento Grid collapsed zu vertikalem Stack. |

**Grid-Tokens:**

| Token | Value | Usage |
|-------|-------|-------|
| `bento-cols` | 4 (Desktop), 2 (Tablet), 1 (Mobile) | Spaltenanzahl. |
| `bento-gap` | `space-4` (16px) | Abstand zwischen Kacheln. |
| `bento-cell-min` | 280px | Minimale Kachelbreite. Darunter collapsed die Spalte. |
| `bento-cell-ratio` | `auto` (Höhe durch Inhalt bestimmt) | Standard-Seitenverhältnis. |
| `bento-span-2` | `grid-column: span 2` | Doppelt breite Kachel (Charts, Tabellen). |
| `bento-span-full` | `grid-column: 1 / -1` | Volle Breite (große Tabellen, Activity-Feed). |
| `bento-cell-featured` | `grid-column: span 2; grid-row: span 2` | Hero-Kachel (Hauptchart, KPI-Fokus). |

**Responsive Collapse:**

```
Desktop (≥1280px):    ┌──┬──┬──┬──┐     4 Spalten
                      ├──┴──┤  ├──┤     span-2 + 1 + 1
                      ├──┬──┼──┴──┤
                      └──┴──┴─────┘

Tablet (768–1279px):  ┌────┬────┐       2 Spalten
                      ├────┴────┤       span-full
                      ├────┬────┤
                      └────┴────┘
...
- Bento Grid wird NUR für die oben genannten Dashboard-Typen verwendet. Neue Bento-Grid-Screens erfordern eine explizite Begründung.
- Maximale Kachel-Anzahl pro Screen: 8 (Desktop), 6 (Tablet). Mehr → Redesign oder Tab-Gruppierung.
- Jede Kachel folgt der Card-Komponente (5.3) mit optionalem Header, Body und Footer.
- Die Kachel-Reihenfolge ist standardmäßig durch AI-Adaptive Layout (6.11) bestimmt, kann aber manuell fixiert werden.
- Kein Drag & Drop in v1. Widget-Reihenfolge wird über Einstellungen konfiguriert, nicht via Drag.
...
| 1.3 | 31.03.2026 | **2026 Design Philosophy Integration.** Light theme confirmed as default; dark theme = toggle via token remapping. Stitch/amber inspiration tokens officially deprecated. Added: Professional Emoji Guidelines (1.10), Detailed Illustrations style guide (4.1), Bottom Navigation Bar for mobile (5.12), Kinetic Typography tokens (3.6.1), Voice Input Pattern (6.9), Preview Panel Pattern (6.10), AI-Adaptive Layout Rules (6.11), Predictive Intelligence UI Patterns (6.12), Adaptive Bento Grid System (7.1). Color resolution clarified: DS v1.3 is the sole canonical source, journey-inline tokens map to DS tokens. Component Spec Index and Screen Spec Index created as companion documents. |
...
|----------|------|-------------|
| **Component Spec Index** | `design/component-specs/_COMPONENT-SPEC-INDEX.md` | Verzeichnis aller Komponenten-Spezifikationen, organisiert nach UI-Kategorie. |
| **Screen Spec Index** | `design/screen-specs/_SCREEN-SPEC-INDEX.md` | Verzeichnis aller Screen-Spezifikationen, organisiert nach PRD-Layer. |
| **Stitch Board Mapping** | `design/google-stitch/STITCH_BOARD_MAPPING.md` | Zuordnung der Stitch-Inspirations-Items zu Journeys und Screens. |
| **Kritische Analyse** | `design/Kritische-Analyse-Design-vs-Product-v1.0.md` | Gap-Analyse zwischen Design-Abdeckung und PRD-Anforderungen. |
