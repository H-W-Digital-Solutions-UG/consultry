...
```mermaid
flowchart TD
    A["Exact Decision Basis v7 and named Client-Steering purpose"] --> B["Responsible applicability of Project, Knowledge/Method/Proof, Brand/CD and Governance/Release bases"]
    B --> C["Alignment-aware revision using current sources and visible gaps"]
    C --> D["Exact successor version considered through three distinguishable Alignment perspectives"]
    D --> E{"Human decides revision and further use"}

    E -->|"Further work needed"| C
    E -->|"No controlled effect yet"| H["Material unresolved matter remains visible"]
    H --> B
    E -->|"Responsible use within Authority"| F["Controlled export, writeback or named use"]
    F --> G["Effect outcome retained with exact version and lineage"]

    G --> I["Possible later basis change and proportionate revalidation"]
    G -.-> J["Optional Corpus Admission or Correction handoff"]
    G -.-> K["Optional ReuseCandidate handoff"]
```
