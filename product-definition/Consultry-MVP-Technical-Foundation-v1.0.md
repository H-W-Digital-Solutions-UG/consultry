...
> Knoten im (relationalen, graph-ready) Modell; Module sind **Views**, keine Silos. Provenance/Source-Bindings + Audit sind an jedem AI-erzeugten Knoten Pflicht.
...
DraftSection 🟢       text, award_criterion_ref? (GI-2 Fit-to-Score),
                      ...
                      citation_links[] (Pflicht bei Firm/External — GI-1/1a),
                      faithfulness_checked: bool (D6)
...
**Enforcement (Pflicht, Datenebene — nicht Frontend):** jeder faktische Output bindet auf `(graph_node_id, span_offset, span_length)` oder `(doc_id, page, span)` zurück. Outputs ohne Binding werden vom Validierungs-Layer **abgelehnt** (`MissingSourceBinding`). Drei-wertige Provenance (GI-1): Firm-Fact → nur Tenant-Korpus; External-Fact → ExternalSource; Model-Expertise → markiert. Default-to-fact bei Klassifikator-Unsicherheit (GI-1a). Faithfulness-Check (D6) prüft, ob die Quelle den Satz trägt. Human-Backstop (GI-1b): der Consultant-Autor verantwortet per Freigabe.
...
| Drift-Monitor | — | 🟡 |
| PII-Routing-Layer | — | 🟡 (MVP: Bedrock-EU für alles) |
