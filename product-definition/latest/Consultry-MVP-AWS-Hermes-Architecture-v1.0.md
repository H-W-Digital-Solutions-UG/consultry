...
**Status:** Technical-Handoff-Architekturplan. ADR-001/002 waren für die frühere MVP-Planung accepted; aktuelle Implementierungsfreigabe erst nach Product-Handoff und Technical Wayfinder.  
**Datum:** 27.06.2026  
**Rolle im Doc-Stack:** Konkretisiert den früheren AWS-nativen Build- und **Hermes-Harness**-Kandidaten: eine kontrollierte Cloud-Sandbox mit streng begrenztem Korpus-Zugriff.  
**Bezug:** [MVP-PRD](../archive/superseded-product-baseline-2026-08/Consultry-MVP-PRD-v1.0.md), [MVP-Technical-Foundation](./Consultry-MVP-Technical-Foundation-v1.0.md), [MVP Architecture ADR](./Consultry-MVP-Architecture-ADR-v1.0.md), [MVP-Foundation-Decisions](./Consultry-MVP-Foundation-Decisions-v1.0.md), [Onboarding-Korpus-Ritual](./Consultry-Onboarding-Corpus-Ritual-v1.0.md).
...
> **Provider-Baseline 12.07.2026:** Diese Datei bleibt bewusst die **AWS-/Bedrock-Referenzvariante**, obwohl auf Product-/MVP-Ebene **Microsoft Foundry / Azure AI Foundry mit GPT-5.6 bevorzugt** ist (`gpt-5.6-sol` für PMF-kritische komplexe Jobs). Bedrock bleibt eine zugelassene Alternative/Fallback-Basiskomponente. Der produktive Model-Contract wird durch `model_policy_id` + provider-neutralen `ModelGateway` definiert; ein Foundry-Adapter/-Deployment braucht einen separaten Infrastruktur-/Identity-/Network-Plan.
...
| A4 | Vektor/Retrieval | App-owned Retrieval in Aurora/pgvector als Source of Truth. Bedrock Knowledge Bases optional fuer Spaeterevaluierung, nicht als autoritative Evidence-/Review-Policy. |
...
| Inferenz | Amazon Bedrock Runtime | Konkreter Alternativ-/Fallback-Pfad dieser AWS-Referenzvariante; der bevorzugte Product-/MVP-Pfad ist Foundry/GPT-5.6 über denselben `ModelGateway`-Vertrag. |
...
| `ResultVerifier` | Prueft Output gegen Schema, Evidence-/Review-Policy und bei verwendeten Quellen gegen SourceMap und Faithfulness. |
...
```json
{
  ...
  "open_issues": [
    {"kind": "missing_source", "severity": "review_required", "text": "..."}
  ]
}
```
...
- **Jeder fachlich relevante Output trägt einen Evidence-/Review-State; SourceBinding ist nach Risk-/Tenant-Policy erforderlich.**
...
### Woche 3 - Retrieval, Evidence State & SourceBinding

- Chunking mit page/span.
- Embeddings via Bedrock.
- pgvector retrieval mit tenant filter.
- Evidence-/Review-State auf Datenebene; CitationLink fuer policy-relevante Claims.
...
| O7 | Bedrock Guardrails als Gate? | Ja als Zusatz, aber nicht statt eigener Evidence-/Review-Policy und Faithfulness-Pruefung. |
...
- Bedrock Knowledge Bases unterstuetzen mehrere Datenquellen/Connectoren; fuer Consultry bleibt App-owned Retrieval trotzdem die autoritative Evidence-Spine.
...
4. **Materielle Claims brauchen Quelle oder sichtbaren Review-Status; es gibt keinen globalen Satz-fuer-Satz-Block.**
5. **Verwendete Quellen muessen auf Dokument, Seite und Spanne zurueckfuehren.**
