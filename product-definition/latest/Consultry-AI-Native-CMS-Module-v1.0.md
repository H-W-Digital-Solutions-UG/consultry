...
### 6.1 Beziehung zu Model-composed Work Surfaces

Das Page-Modul ist eine spezialisierte Output-Kante des allgemeineren Consultry-Prinzips der `Model-composed Work Surface`: Modelle können typisierte Inhalte und die Komposition freigegebener UI-Komponenten erzeugen, die Consultry für den Nutzer interaktiv rendert. QA-Fragensets, Evidence-/Decision-Frames oder Artifact-Arbeitsflächen bleiben dabei App-interne Work Surfaces; `PageBlueprint`, `PageSection` und `PageVariant` sind dagegen publizierbare Output-Artefakte mit zusätzlichen Brand-, Proof-, Publication- und Approval-Gates.

Beide dürfen denselben kontrollierten Component-/Design-System-Renderer verwenden. Eine dynamisch gerenderte Work Surface impliziert jedoch weder eine öffentliche Website noch Publication Authority; das CMS-Modul bleibt ein späterer Suite Candidate und wird durch das Rendering-Prinzip nicht in H1 gezogen.
...
| Unbelegte Marketingclaims | materielle Proof-, Capability-, Preis- oder Rechtsclaims zeigen SourceBinding oder offenen Review-Status; Publish kann policy-gated sein |
...
| CMS-04 | BrandPageHarnessPack | pack builder for brand, language, design, offer, proof | 900 | missing proof creates review-required claim |
| CMS-05 | PageCompiler v0 | blueprint + section generation against schema | 1000 | deterministic fixture output |
| CMS-06 | ClaimVerifier | EvidenceState, optional SourceBinding and risk classification for page sections | 900 | unsupported High-Risk claim cannot publish until reviewed |
