...
**Status:** Previously accepted architecture baseline; Technical-Handoff candidate pending product definition and later Technical Wayfinder  
**Date:** 28.06.2026  
**Role in doc stack:** Preserves the prior P0 architecture decisions for later technical confirmation or revision; it does not freeze current Product Scope or Business-Domain truth.  
...
- **Managed AI Platform baseline (revidiert 12.07.2026):** **Microsoft Foundry / Azure AI Foundry ist bevorzugt.** Preferred Family = GPT-5.6; `gpt-5.6-sol` ist Default für PMF-kritische komplexe Reasoning-/Synthese-Jobs, initial auf Version `2026-07-09` gepinnt. Terra/Luna sind optionale eval-gated Routen. Bedrock bleibt zugelassene Alternative/Fallback hinter demselben provider-neutralen `ModelGateway`- und Compliance-Contract. Die AWS-native Datenbaseline dieses ADR bleibt davon unberührt; Provider-/Modellwahl erfolgt über versionierte `model_policy_id`-Konfiguration.
