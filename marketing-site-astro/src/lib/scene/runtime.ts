...
export function createSmokeScene(host: HTMLElement, options: SmokeSceneOptions): SmokeScene {
  ...
  const measureSignals = () => {
    ...
    signalConnections.forEach((connection) => {
      ...
      connection.x = connection.note.offsetLeft + (left ? connection.note.offsetWidth : 0);
      connection.y = connection.note.offsetTop + 18;
  ...
  const render = (now: number) => {
    ...
    try {
      ...
      const moving = frameProgress > 0 && frameProgress < 1;
      ...
      const poseChanged = model.update(frameProgress === 1 ? 0 : frameProgress);
      const needsDraw = forceDraw || !ready || bufferChanged || poseChanged;
    ...
    if (displayedProgress !== target) requestFrame();
  };
  const requestFrame = () => {
