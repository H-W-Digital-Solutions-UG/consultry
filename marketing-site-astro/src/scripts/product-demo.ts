...
function mountDemo(root: HTMLElement) {
  ...
  const paint = () => {
    ...
    toggleIcon.dataset.playbackState =
      phase === "playing" ? "pause" : phase === "complete" ? "replay" : "play";
    toggle.setAttribute("aria-label", `${t("Beispielablauf")}: ${t(label)}`);
  ...
  viewButtons.forEach(button => button.addEventListener("click", () => {
    ...
    paint();
    announcer.textContent = `${t(view === "basis" ? "Ausgangsstand" : "Arbeitsstand")}: ${version?.textContent || ""}`;
    event(`view:${view}`);
  }, opts));
