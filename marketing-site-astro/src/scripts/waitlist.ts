...
function connectForm(form: HTMLFormElement) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    button.textContent = "Wird eingetragen …";
    ...
    try {
      ...
      report("Anmeldung erhalten. Bitte bestätige den Link in deiner E-Mail, um deinen Eintrag abzuschließen.", "success");
      status.tabIndex = -1;
      status.focus();
    } catch {
      ...
      if (!accepted) { button.disabled = false; button.innerHTML = buttonHtml; }
    }
  });
}
