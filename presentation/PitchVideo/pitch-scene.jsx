...
const OFFER_EDIT_SIDEBAR = {
  x: APP_CONTENT_LEFT + APP_CONTENT_W - 540,
  y: APP_CONTENT_TOP,
  w: 540,
  h: APP_CONTENT_H,
};
...
const OFFER_OVERVIEW_HOLD = 0.65; // let Deal closed resolve into the AWS Transformation overview before prompt typing
...
const SCENE_VERTRIEB_START = 32.05 + DOWNSTREAM_SCENE_DELAY - OPENING_SEQUENCE_CUT - TEAM_PROFILE_SEQUENCE_CUT - TEAM_DIRECT_VIEW_CUT - TEAM_STAFFING_SEQUENCE_CUT;
const SCENE_VERTRIEB_END = 53.15 + DOWNSTREAM_SCENE_DELAY + WORK_SEQUENCE_SHIFT + 2.90 - PROMPT_SEQUENCE_ADVANCE - OPENING_SEQUENCE_CUT - TEAM_PROFILE_SEQUENCE_CUT - TEAM_DIRECT_VIEW_CUT - TEAM_STAFFING_SEQUENCE_CUT + OFFER_OVERVIEW_HOLD; // keep prompt scene alive through agent packet handoff
...
const SCENE_WISSEN_START = 53.15 + DOWNSTREAM_SCENE_DELAY + WORK_SEQUENCE_SHIFT - PROMPT_SEQUENCE_ADVANCE - OPENING_SEQUENCE_CUT - TEAM_PROFILE_SEQUENCE_CUT - TEAM_DIRECT_VIEW_CUT - TEAM_STAFFING_SEQUENCE_CUT - PROMPT_IDLE_SEQUENCE_CUT + OFFER_OVERVIEW_HOLD;
const SCENE_WISSEN_END = 70.90 + DOWNSTREAM_SCENE_DELAY + WORK_SEQUENCE_SHIFT - PROMPT_SEQUENCE_ADVANCE - OPENING_SEQUENCE_CUT - TEAM_PROFILE_SEQUENCE_CUT - TEAM_DIRECT_VIEW_CUT - TEAM_STAFFING_SEQUENCE_CUT - PROMPT_IDLE_SEQUENCE_CUT + OFFER_OVERVIEW_HOLD;
...
const SCENE_MATCHED_START = SCENE_WISSEN_START + 2.90;   // completed agent message lands before the matched-output reveal
const SCENE_MATCHED_END = SCENE_MATCHED_START + 5.00;
const SCENE_FINANZ_START = SCENE_MATCHED_END - 0.50;
const SCENE_FINANZ_END = SCENE_FINANZ_START + 9.00;
const WORKFLOW_CANVAS_UI_START = 35.00;
const WORKFLOW_CANVAS_UI_END = 50.00;
...
const MUSIC_MUTED = false; // soundtrack remains active
...
function AppMouseCursor({ t, fade = 1 }) {
  const inSolution = t < SCENE_VERTRIEB_START;
  const inVertrieb = t >= SCENE_VERTRIEB_START && t < SCENE_MATCHED_START;
  const inMatched = t >= SCENE_MATCHED_START;
  const cursorIn = rise(t, SIGNAL_START + SIGNAL_BEAT_FEED + 0.08, 0.52);
  const cursorOut = rise(t, SCENE_FINANZ_START - 0.55, 0.50);
  const op = fade * cursorIn * (1 - cursorOut);
  ...
  if (inSolution) {
    ...
    const points = [
      { t: SIGNAL_BEAT_FEED + 0.05, x: 1472, y: 258 },
      { t: SIGNAL_BEAT_SELECT + 0.18, x: 1054, y: 558 },
      { t: SIGNAL_BEAT_DKB_ACTIVE + 0.42, x: 944, y: 514 },
      { t: SIGNAL_TEAM_PROFILE_STAGE_START - 0.28, x: 1042, y: 402 },
      { t: SIGNAL_TEAM_PROFILE_STAGE_START + 1.10, x: 668, y: 494 },
      { t: SIGNAL_TEAM_OUTREACH_STAGE_START + 0.36, x: 1176, y: 448 },
      { t: SIGNAL_TEAM_OUTREACH_STAGE_START + 1.12, x: 1510, y: 514 },
      { t: SIGNAL_TEAM_STAFFING_PAGE_START + 0.44, x: 1078, y: 642 },
      { t: LOGO_BRIDGE_START - SIGNAL_START + 0.52, x: 1114, y: 512 },
    ];
    p = sampleMotionPath(points, l);
    click = Math.max(
      band(l, SIGNAL_BEAT_SELECT + 0.16, SIGNAL_BEAT_SELECT + 0.74, 0.13),
      band(l, SIGNAL_TEAM_PROFILE_STAGE_START - 0.18, SIGNAL_TEAM_PROFILE_STAGE_START + 0.40, 0.13),
      band(l, SIGNAL_TEAM_OUTREACH_STAGE_START + 0.20, SIGNAL_TEAM_OUTREACH_STAGE_START + 0.78, 0.13),
      band(l, SIGNAL_TEAM_STAFFING_PAGE_START + 0.18, SIGNAL_TEAM_STAFFING_PAGE_START + 0.76, 0.13),
    );
  } else if (inVertrieb) {
    const l = t - SCENE_VERTRIEB_START;
    const points = [
      { t: 0.20, x: 1474, y: 174 },
      { t: 2.15, x: 1034, y: 510 },
      { t: Math.max(3.1, OFFER_BG_START + 0.88), x: 760, y: 610 },
      { t: WORK_PROMPT_PANEL_START - 0.34, x: 1034, y: 558 },
      { t: WORK_PROMPT_PANEL_START + 0.42, x: 824, y: 438 },
      { t: WORK_PROMPT_SELECT_START + 0.08, x: 1546, y: 330 },
      { t: WORK_PROMPT_VISUAL_START + 0.74, x: 1456, y: 388 },
      { t: WORK_PROMPT_VISUAL_START + PROMPT_BRIDGE_COLLECT_OFFSET + 0.76, x: 1328, y: 482 },
      { t: WORK_DETAIL_START + 0.56, x: 1188, y: 444 },
      { t: WORK_DETAIL_END - 0.24, x: 1516, y: 698 },
    ];
    p = sampleMotionPath(points, l);
    click = Math.max(
      band(l, 1.95, 2.48, 0.13),
      band(l, WORK_PROMPT_PANEL_START + 0.30, WORK_PROMPT_PANEL_START + 0.80, 0.13),
      band(l, WORK_PROMPT_SELECT_START - 0.08, WORK_PROMPT_SELECT_START + 0.42, 0.13),
      band(l, WORK_DETAIL_START + 0.42, WORK_DETAIL_START + 0.96, 0.13),
    );
  } else if (inMatched) {
    const l = t - SCENE_MATCHED_START;
    const points = [
      { t: 0.00, x: 736, y: 362 },
      { t: 0.72, x: 752, y: 374 },
      { t: 1.52, x: 752, y: 430 },
    ...
    p = sampleMotionPath(points, l);
    click = Math.max(
      band(l, 0.62, 1.08, 0.13),
      band(l, 1.42, 1.88, 0.13),
    );
  } else {
    p = { x: 1500, y: 180 };
  }
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 58, opacity: op }}>
      <div style={{ position: 'absolute', left: p.x, top: p.y,
        transform: `translate(-4px,-2px) scale(${1 - click * 0.08})`,
        filter: 'drop-shadow(0 0 8px rgba(143,191,216,0.72)) drop-shadow(0 10px 18px rgba(0,0,0,0.38))' }}>
        <svg width="36" height="43" viewBox="0 0 36 43" fill="none">
          <path d="M4.1 3.7 29.4 24.9 18.2 26.5 12.4 39.1 4.1 3.7Z"
            fill="#050607" stroke="rgba(255,255,255,0.94)" strokeWidth="3.2" strokeLinejoin="round" />
          <path d="M4.1 3.7 29.4 24.9 18.2 26.5 12.4 39.1 4.1 3.7Z"
            fill="#050607" stroke="rgba(6,9,10,0.92)" strokeWidth="1.35" strokeLinejoin="round" />
          <path d="M18.4 26.4 28.8 37.8" stroke="rgba(255,255,255,0.92)" strokeWidth="4.2" strokeLinecap="round" />
          <path d="M18.4 26.4 28.8 37.8" stroke="#050607" strokeWidth="2.2" strokeLinecap="round" />
        </svg>
      </div>
      {click > 0.01 && (
        <div style={{ position: 'absolute', left: p.x + 8, top: p.y + 8,
          width: 86, height: 86, borderRadius: 999,
          transform: `translate(-50%,-50%) scale(${0.44 + click * 0.76})`,
          opacity: (1 - Math.abs(click - 0.5) * 1.2) * 0.74,
          border: '2px solid rgba(240,168,94,0.72)',
          boxShadow: '0 0 28px rgba(240,168,94,0.28), inset 0 0 18px rgba(240,168,94,0.16)' }} />
      )}
    </div>
  );
}
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', opacity: shellOpacity, zIndex: 45 }}>
      <div style={{ position: 'absolute', left: APP_FRAME_LEFT, top: APP_FRAME_TOP, width: APP_FRAME_W, height: APP_FRAME_H,
        ...
        <div style={{ position: 'absolute', left: 0, top: APP_TOPBAR_H, bottom: 0, width: APP_SIDEBAR_W,
          background: APP_SURFACE,
          borderRight: `1px solid ${APP_LINE}`,
          boxShadow: '18px 0 34px rgba(0,0,0,0.20)' }}>
          <div style={{ position: 'absolute',
            left: APP_SIDEBAR_ITEM_LEFT + APP_SIDEBAR_ITEM_SIZE / 2 - 1,
            top: APP_SIDEBAR_NAV_TOP + APP_SIDEBAR_ITEM_SIZE / 2,
            width: 2,
...
const SIGNAL_TEAM_STAFFING_STAGE_START = SIGNAL_TEAM_PROFILE_STAGE_START + SIGNAL_TEAM_PROFILE_STAGE_DUR + 1.05;
const SIGNAL_TEAM_STAFFING_STAGE_DUR = 2.40 - TEAM_STAFFING_SEQUENCE_CUT;
...
const SIGNAL_TEAM_PROFILE_EXIT_START = SIGNAL_TEAM_OUTREACH_STAGE_START - 0.38;
...
function SignalGraphPanel({ local }) {
  ...
  const signalEvents = [
    ...
    { tag: 'Financial Times', title: 'Bank AG wechselt zu AWS Cloud', meta: 'Cloud-Migration', icon: 'financial-times', color: '#b87842' },
  ...
  const selectedRow = {
    x: cardX + 38,
    y: cardY + eventListTop + eventRowH + eventGap,
    w: cardW - 76,
    h: eventRowH,
  };
    ...
    <div style={{ position: 'absolute', left: '50%', top: 0, width: 1660, height: panelH,
      ...
      <svg width="1660" height={panelH} viewBox={`0 0 1660 ${panelH}`} style={{ position: 'absolute', inset: 0, overflow: 'visible' }}>
        ...
        {SIGNAL_TREE.map((node, i) => {
          ...
          const feedTop = cardY + eventListTop + 24;
          const feedBottom = cardY + cardH - 70;
          const targetY = leftSide
            ? feedTop + i * ((feedBottom - feedTop) / Math.max(1, SIGNAL_TREE.length - 1))
      ...
      <div style={{ position: 'absolute', left: cardX, top: cardY, width: cardW, height: cardH, zIndex: 5,
        borderRadius: 0, padding: 0, boxSizing: 'border-box',
        ...
        <div style={{ position: 'absolute', left: 0, top: 0, right: 0, height: 56,
          opacity: documentUi, display: 'grid', gridTemplateColumns: '1fr auto',
          alignItems: 'center', columnGap: 14 }}>
          <div style={{ height: 54, borderRadius: 18,
            display: 'flex', alignItems: 'center', gap: 15, padding: '0 18px',
            boxSizing: 'border-box', background: APP_SURFACE,
            border: `1px solid ${APP_LINE}`, boxShadow: '0 12px 28px rgba(45,38,32,0.06)' }}>
            <Icon name="search" size={22} color="#5f9fc0" sw={2.2} />
            <span style={appTyped({ fontSize: 17, fontWeight: 760 })}>
              Bank AG AWS Cloud Migration
            </span>
            <span style={{ width: 2, height: 26, borderRadius: 99, marginLeft: 2,
              background: '#d99a58', opacity: 0.72 }} />
          </div>
          <div style={{ height: 50, borderRadius: 999, padding: '0 18px',
            display: 'flex', alignItems: 'center', gap: 10, boxSizing: 'border-box',
            background: APP_RAISED, border: `1px solid ${APP_LINE}`,
            fontFamily: INTER, fontWeight: 760, fontSize: 14, color: APP_TEXT }}>
            <span style={{ width: 7, height: 7, borderRadius: 99,
              background: '#7fdca6', boxShadow: '0 0 10px rgba(127,220,166,0.36)' }} />
            3 aktive Signale
          </div>
        </div>
        ...
        <div style={{ position: 'absolute', left: 38, right: 38, top: eventListTop, bottom: 42,
          ...
          <div style={{ position: 'absolute', left: 0, right: 0, top: 10,
            ...
            {signalEvents.map((event, idx) => {
              ...
              return (
                <div key={event.title} style={{ height: eventRowH, borderRadius: 18,
                  ...
                  boxShadow: idx === 1
                    ? `0 14px 30px rgba(45,38,32,0.10), 0 0 ${18 + dkbActivate * 20}px rgba(184,120,66,${0.08 + dkbActivate * 0.14})`
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: MONO, fontSize: 9.6, letterSpacing: '0.13em',
                      textTransform: 'uppercase', color: APP_MUTED }}>{event.tag}</div>
                    <div style={{ marginTop: 5, fontFamily: DISPLAY, fontWeight: 780, fontSize: 24,
                      letterSpacing: '-0.012em', color: APP_TEXT, whiteSpace: 'nowrap' }}>{event.title}</div>
                    <div style={{ marginTop: 4, fontFamily: INTER, fontWeight: 680, fontSize: 12.2,
                      lineHeight: 1.12, color: APP_MUTED, whiteSpace: 'nowrap' }}>{event.meta}</div>
                  </div>
      ...
      <div style={{ position: 'absolute', left: cardX + cardW + 46, top: cardY + 2,
        width: 430, height: 484, zIndex: 5,
        opacity: documentUi * (1 - frameExit * 0.98),
        transform: `translateY(${(1 - active) * 10 + frameExit * 8}px)`,
        display: 'grid', gridTemplateRows: 'auto 1fr 1fr 1fr', rowGap: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '0 2px 4px 2px' }}>
          <div>
            <div style={{ fontFamily: INTER, fontWeight: 830, fontSize: 26,
              lineHeight: 1, color: APP_TEXT, letterSpacing: '-0.014em' }}>
              Signal-Auswertung
            </div>
            <div style={{ marginTop: 7, fontFamily: INTER, fontWeight: 650,
              fontSize: 13.5, color: APP_MUTED }}>
          ...
          <div style={{ width: 42, height: 42, borderRadius: 14,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            background: APP_RAISED, border: `1px solid ${APP_LINE}` }}>
            <Icon name="target" size={22} color={WARM} sw={2.1} />
          </div>
        </div>
        {[
          { icon: 'briefcase', label: 'Kunde', title: 'Bank AG', meta: 'neues Cloud-Programm', color: '#b87842' },
          { icon: 'zap', label: 'Trigger', title: 'AWS Migration', meta: 'Projektbedarf steigt', color: '#5f9fc0' },
          { icon: 'shield', label: 'Priorität', title: 'Security + DORA', meta: 'relevant für Angebot', color: '#c35bb9' },
        ].map((item, idx) => {
          const p = rise(local, SIGNAL_BEAT_FEED + 0.42 + idx * 0.18, 0.62);
          return (
            <div key={item.label} style={{ borderRadius: 22,
              display: 'grid', gridTemplateColumns: '62px 1fr auto',
              alignItems: 'center', gap: 14, padding: '16px 18px',
              boxSizing: 'border-box', background: APP_SURFACE,
              border: `1px solid ${APP_LINE}`,
              boxShadow: '0 16px 34px rgba(45,38,32,0.07)',
              ...
              <div style={{ width: 54, height: 54, borderRadius: 18,
                ...
                <Icon name={item.icon} size={24} color={item.color} sw={2.1} />
              </div>
              <div>
                <div style={{ fontFamily: INTER, fontWeight: 710, fontSize: 12.5,
                  color: APP_MUTED }}>{item.label}</div>
                <div style={{ marginTop: 5, fontFamily: DISPLAY, fontWeight: 820,
                  fontSize: 22, color: APP_TEXT, letterSpacing: '-0.012em' }}>
                  {item.title}
                </div>
                <div style={{ marginTop: 4, fontFamily: INTER, fontWeight: 640,
                  fontSize: 12.6, color: APP_MUTED }}>
                  {item.meta}
                </div>
              </div>
      ...
      {(() => {
          ...
          return (
            <div style={{ position: 'absolute', left: bridgeRow.x, top: bridgeRow.y,
              ...
              boxShadow: `0 26px 68px rgba(45,38,32,${0.10 + detail * 0.04}), 0 0 ${16 + activePulse * 14 + detail * 24}px rgba(240,168,94,${0.07 + activePulse * 0.035 + detail * 0.02}), inset 0 1px 0 rgba(255,255,255,0.74)`,
              overflow: 'hidden' }}>
              ...
              <div style={{ position: 'absolute', left: titleLeft, top: titleTop,
                ...
                <div style={{ fontFamily: MONO, fontSize: mix(9.6, 11.0, detail), letterSpacing: '0.13em',
                  textTransform: 'uppercase', color: APP_MUTED }}>Financial Times</div>
              ...
              <div style={{ position: 'absolute', left: 38, right: 38, top: 222,
                height: 166, opacity: detailBody * detail,
                display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
                {[
                  { icon: 'briefcase', label: 'Account', title: 'Bank AG', meta: 'Cloud-Programm aktiv', color: '#b87842' },
                  { icon: 'zap', label: 'Trigger', title: 'AWS Migration', meta: 'Priorität gestiegen', color: '#5f9fc0' },
                  { icon: 'shield', label: 'Risiko', title: 'Security + DORA', meta: 'Angebotsrelevant', color: '#c35bb9' },
                ].map((item, idx) => (
                  <div key={item.label} style={{ borderRadius: 18, padding: '16px 16px 14px',
                    boxSizing: 'border-box', background: APP_RAISED,
                    border: `1px solid ${APP_LINE}`,
                    boxShadow: '0 12px 24px rgba(45,38,32,0.06)',
                    transform: `translateY(${(1 - detailBody) * 8}px)`,
                    opacity: detailBody * (0.74 + idx * 0.08) }}>
                    <div style={{ width: 42, height: 42, borderRadius: 14,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      background: `${item.color}12`, border: `1px solid ${item.color}26` }}>
                      <Icon name={item.icon} size={21} color={item.color} sw={2.1} />
                    </div>
                    <div style={{ marginTop: 14, fontFamily: INTER, fontWeight: 720,
                      fontSize: 12.2, color: APP_MUTED }}>{item.label}</div>
                    <div style={{ marginTop: 5, fontFamily: DISPLAY, fontWeight: 820,
                      fontSize: 20, lineHeight: 1.02, color: APP_TEXT }}>{item.title}</div>
                    <div style={{ marginTop: 6, fontFamily: INTER, fontWeight: 640,
                      fontSize: 12, color: APP_MUTED }}>{item.meta}</div>
                  </div>
                ))}
              </div>
              <div style={{ position: 'absolute', left: 38, right: 38, top: 406,
                height: 50, opacity: detailBody * detail,
                borderRadius: 16, background: APP_RAISED,
                border: `1px solid ${APP_LINE}`, display: 'flex', alignItems: 'center',
                gap: 10, padding: '0 16px', boxSizing: 'border-box',
                fontFamily: INTER, fontWeight: 720, fontSize: 13.5, color: APP_TEXT }}>
                {['Quelle geprüft', 'Projektkontext verknüpft', 'Nächster Angebotsschritt'].map((label, idx) => (
                  <span key={label} style={{ height: 28, borderRadius: 999,
                    padding: '0 11px', display: 'inline-flex', alignItems: 'center',
                    gap: 7, background: '#fffaf3', border: `1px solid ${APP_LINE}` }}>
                    <span style={{ width: 6, height: 6, borderRadius: 99,
                      background: idx === 0 ? '#8fbfd8' : idx === 1 ? WARM : '#e8655a' }} />
                    {label}
                  </span>
                ))}
              </div>
...
function TeamMorphModules({ local, showAttention = true }) {
  ...
  const profileSceneOpacity = previewVisible * (1 - profileSceneExit);
  const outreachSceneExit = rise(local, staffingStageStart - 0.24, 0.48);
  const outreachSceneOpacity = previewVisible *
    band(local, matchStageStart - 0.04, staffingStageStart + 0.28, 0.58) *
    (1 - outreachSceneExit * 0.96);
  const staffingSceneOpacity = previewVisible * rise(local, SIGNAL_TEAM_STAFFING_VISUAL_START, 0.58);
  ...
  const rightTo = { x: 76, y: 76, w: 1488, h: 638 };
  const profileListTo = { x: 88, y: 66, w: 1464, h: 528 };
  ...
  const profile = [
    ...
    { short: 'KOM', value: 0.76, color: '#c65bb0' },
  ];
  ...
  const roles = [
    { initials: 'SA', name: 'Sr. Architect - Max Muster', fit: '96%', focus: 'Azure Security', certs: ['AZ-500', 'AWS-SAA'],
    ...
    { initials: 'SE', name: 'Sec. Engineer - Lena Weber', fit: '91%', focus: 'Sentinel / SOC', certs: ['SC-200', 'AWS-SCS'],
    ...
    { initials: 'DL', name: 'Delivery Lead - Nora Schulz', fit: '78%', focus: 'Delivery Risk', certs: [],
      ...
      avatar: 1, skin: '#cfa17f', hair: '#352728' },
  ];
  ...
  const dateTicks = [
    ...
    { label: 'W2', sub: 'Tag 5', p: 0.34 },
    { label: 'W3', sub: 'Tag 10', p: 0.68 },
    { label: 'W4', sub: 'Tag 15', p: 1.00 },
  ];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: enter * (1 - bundleOut * 0.78),
      ...
      <div style={{ position: 'absolute', ...left, borderRadius: 0,
          ...
          <div style={{ opacity: leftMorph,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              {requirementFacts.map((fact, i) => {
                ...
                return (
                  <div key={fact.label} style={{ height: mix(58, 74, leftMorph), borderRadius: 0,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {requirementDetails.map((item, i) => {
                ...
                return (
                  <div key={item.title} style={{ height: mix(72, 92, leftMorph), borderRadius: 0,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 0.85fr', gap: 14 }}>
              <div style={{ height: mix(128, 146, leftMorph), borderRadius: 0, padding: '16px 18px',
              ...
              <div style={{ height: mix(128, 146, leftMorph), borderRadius: 0, padding: '16px 18px',
                ...
                <div style={{ marginTop: 15, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                  {[
                    ...
                    return (
                      <div key={status.label} style={{ height: 72, borderRadius: 0,
      ...
      <div style={{ position: 'absolute', ...right, borderRadius: 0,
        ...
        <div style={{ position: 'absolute', left: mix(32, 6, rightMorph), top: mix(24, 18, rightMorph),
          ...
          <div style={{ fontFamily: DISPLAY, fontWeight: 790,
            fontSize: mix(20, 35, rightMorph), color: APP_TEXT, letterSpacing: '-0.026em', lineHeight: 0.98 }}>AWS Cloud Transformation</div>
        ...
        <div style={{ position: 'absolute', right: mix(32, 80, rightMorph), top: mix(28, 16, rightMorph), height: 30,
        ...
        <div style={{ position: 'absolute',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `${mix(276, 348, rightMorph)}px 1fr ${mix(136, 172, rightMorph)}px`,
            ...
            <div style={{ borderRadius: 0, background: APP_SURFACE, border: `1px solid ${APP_LINE_STRONG}`,
            ...
            <div style={{ borderRadius: 0, background: APP_SURFACE, border: `1px solid ${APP_LINE}`,
              padding: '0 14px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', alignItems: 'center' }}>
              {dateTicks.map((tick, i) => (
                <div key={`staffing-board-date-${tick.label}`} style={{ minWidth: 0,
                  paddingLeft: i === 0 ? 2 : 14, borderLeft: i === 0 ? 'none' : `1px solid ${APP_LINE}` }}>
        ...
        background: APP_SURFACE,
        border: `1px solid ${APP_LINE}`,
        boxShadow: `0 24px 58px rgba(45,38,32,0.12), inset 3px 0 0 rgba(143,191,216,${0.18 + matchBeat * 0.20})`,
        overflow: 'hidden' }}>
        <div style={{ position: 'absolute', left: 22, right: 22, top: 22, height: 76,
          ...
          <div style={{ height: 76, borderRadius: 0, display: 'flex', alignItems: 'center', gap: 16,
            padding: '0 20px', boxSizing: 'border-box',
            background: APP_RAISED,
            border: `1px solid ${APP_LINE_STRONG}`,
            boxShadow: 'inset 2px 0 0 rgba(143,191,216,0.46)' }}>
        ...
        <div style={{ position: 'absolute', left: 22, right: 22, top: 122, display: 'flex',
          flexDirection: 'column', gap: 16 }}>
          {matchedConsultants.map((person, i) => {
            const cp = rise(local, matchT(i) + 0.16, 0.92);
            ...
            const shortName = person.name
              ...
              .replace('Platform Eng. - Jonas Klein', 'Platform Eng. - Jonas');
            return (
              <div key={person.name} style={{ height: 104, borderRadius: 0,
                padding: '14px 22px 14px 16px', boxSizing: 'border-box',
		                display: 'grid', gridTemplateColumns: '44px 66px 360px 80px 238px 1fr', columnGap: 14,
		                alignItems: 'center', opacity: cp,
                transform: `translateX(${(1 - cp) * 28}px) translateY(${-selectedP * 3}px) scale(${1 + selectedP * 0.018})`,
                background: APP_RAISED,
                border: `1px solid ${selectedP > 0.08 ? APP_LINE_STRONG : APP_LINE}`,
                boxShadow: `0 0 ${selectedP * 18}px ${APP_ACCENT}18, inset 3px 0 0 ${selectedP > 0.08 ? APP_ACCENT : person.color}` }}>
                <div style={{ width: 36, height: 36, borderRadius: 999,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: MONO, fontSize: 11.5, fontWeight: 820,
                  letterSpacing: '0.04em', color: person.color,
                  background: `${person.color}12`, border: `1px solid ${person.color}32` }}>
                ...
                <div style={{ width: 64, height: 64, borderRadius: 999, display: 'flex',
                  ...
                  <ConsultantAvatar person={person} size={64} selected={selectedP} />
                    ...
                    <span style={{ fontFamily: DISPLAY, fontWeight: 745, fontSize: 26,
                ...
                <div style={{ height: 34, borderRadius: 999, display: 'flex',
                  alignItems: 'center', justifyContent: 'center',
                  fontFamily: MONO, fontSize: 11.5, fontWeight: 820,
                  letterSpacing: '0.045em', color: person.color,
                  background: `${person.color}13`, border: `1px solid ${person.color}30` }}>
                  {person.fit}
                </div>
	                <div style={{ minWidth: 0, height: 34, display: 'flex',
	                  alignItems: 'center', justifyContent: 'flex-start', gap: 6, opacity: 0.94 }}>
	                  {(person.certs || []).length > 0 ? (
	                    (person.certs || []).map((cert) => (
	                      <CertificationBadge key={cert} cert={cert} color={person.color} mini />
	                    ))
	                  ) : (
	                    <span style={{ width: 88, height: 27, borderRadius: 999,
	                      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
	                      opacity: 0.28, border: `1px solid ${person.color}24`,
	                      background: APP_RAISED }}>
	                      <Icon name="shield" size={15} color={person.color} sw={1.7} />
	                    </span>
	                  )}
	                </div>
	                <div style={{ position: 'relative', height: 34 }}>
	                  <div style={{ position: 'absolute', left: 0, right: 0, top: 17, height: 2,
	                    borderRadius: 99, background: APP_LINE }} />
	                  {(person.occupied || []).map((slot, j) => (
	                    <div key={`${person.initials}-profile-busy-${j}`} style={{ position: 'absolute',
	                      left: `${slot.start * 100}%`, top: 12,
	                      width: `${slot.span * 100}%`, height: 10, borderRadius: 99,
                    ...
                    transform: `scaleX(${cp})`, transformOrigin: '0 50%' }} />
                  <span style={{ position: 'absolute', right: 0, top: 12, width: 10, height: 10,
          ...
          <div style={{ position: 'absolute', left: 50, top: 112, width: 682, height: 536,
            ...
            <div style={{ position: 'absolute', left: -150, top: -92, width: 982, height: 720,
              ...
              <svg width="982" height="720" viewBox="0 0 982 720"
                ...
                <ellipse cx="491" cy="364" rx="402" ry="272" fill="url(#project-canvas-focus)" />
                ...
                <circle cx="491" cy="360" r="286" fill="none" stroke="rgba(116,198,157,0.15)"
                  strokeWidth="1.4" strokeDasharray="10 22" />
                <circle cx="491" cy="360" r="210" fill="none" stroke="rgba(143,191,216,0.14)"
                  strokeWidth="1.2" strokeDasharray="18 28" />
              </svg>
            </div>
            <svg width="682" height="536" viewBox="0 0 682 536"
              ...
              {laidOutContextNodes.map((node, i) => {
                ...
                const side = node.ax < 341 ? -1 : 1;
                return (
                  <g key={`${node.k}-line`} opacity={p}>
                    <path d={`M 341 268 C ${(341 + node.ax) / 2} ${268 - 62}, ${(341 + node.ax) / 2 + side * 18 * inlineLayout} ${node.ay}, ${node.ax} ${node.ay}`}
                      fill="none" stroke={APP_LINE_STRONG} strokeWidth="8" strokeLinecap="round" />
                    <path d={`M 341 268 C ${(341 + node.ax) / 2} ${268 - 62}, ${(341 + node.ax) / 2 + side * 18 * inlineLayout} ${node.ay}, ${node.ax} ${node.ay}`}
              ...
              <circle cx="341" cy="268" r="148" fill="rgba(143,191,216,0.035)"
                stroke={APP_LINE_STRONG} strokeDasharray="8 18" opacity="0.60" />
              <circle cx="341" cy="268" r="102" fill="rgba(143,191,216,0.055)"
                stroke="rgba(143,191,216,0.16)" opacity="0.68" />
            </svg>
            ...
            {laidOutContextNodes.map((node) => {
              ...
              return (
                <div key={node.k} style={{ position: 'absolute', left: node.x, top: node.y,
                  width: node.w, height: node.h, borderRadius: 18, padding: '15px 16px',
                  boxSizing: 'border-box', display: 'grid', gridTemplateColumns: '84px 1fr',
                  alignItems: 'center', columnGap: 14,
                  ...
                  background: APP_SURFACE,
                  border: `1px solid ${node.color}${active > 0.01 ? '70' : '36'}`,
                  boxShadow: `0 16px 34px rgba(45,38,32,0.10), inset 0 1px 0 rgba(255,255,255,0.68), 0 0 ${active * 18}px ${node.color}30` }}>
                  <span style={{ position: 'absolute', left: node.x < 300 ? 'auto' : -7,
                    right: node.x < 300 ? -7 : 'auto', top: '50%', width: 12, height: 12,
                    borderRadius: 99, transform: 'translateY(-50%)',
                    background: APP_SURFACE, border: `2px solid ${node.color}`,
                    boxShadow: `0 0 10px ${node.color}44` }} />
                  <div style={{ position: 'absolute', right: 12, top: 10, display: 'grid',
                    gridTemplateColumns: 'repeat(3, 3px)', gap: 3, opacity: 0.45 }}>
                    {[0, 1, 2, 3, 4, 5].map((dot) => (
                      <span key={dot} style={{ width: 3, height: 3, borderRadius: 99,
                        background: APP_FAINT }} />
                    ))}
                  </div>
                  <WorkArtifactPreview type={node.mock} color={node.color} active={active} />
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: DISPLAY, fontWeight: 760, fontSize: 25,
                      lineHeight: 1.02, color: APP_TEXT, letterSpacing: '-0.01em',
                      whiteSpace: 'nowrap',
...
function VertriebEditSidebar({ local, state = offerEditSidebarState(local) }) {
  ...
  const commentThreads = [
    {
      author: 'Consultry Intelligence',
      text: 'Projektpaket analysiert. Meeting-Fragen, Risiken und nächste Schritte sind ableitbar.',
      color: '#8fbfd8',
    },
    {
      author: 'Lena',
      text: 'Bitte Status-Meeting mit BankAG vorbereiten und offene Security-Punkte priorisieren.',
      color: WARM,
    },
  ];
  const referenceItems = [
    ...
    { label: 'Security Scope', meta: 'IAM / GuardDuty', icon: 'shield', color: '#8fbfd8' },
    { label: 'Risiko Review', meta: 'DORA / BaFin', icon: 'clock', color: '#e8655a' },
    { label: 'Meeting Ziel', meta: 'Status', icon: 'target', color: '#c65bb0' },
  ];

  return (
    <div style={{ position: 'absolute',
      left: OFFER_EDIT_SIDEBAR.x, top: OFFER_EDIT_SIDEBAR.y,
      width: OFFER_EDIT_SIDEBAR.w, height: OFFER_EDIT_SIDEBAR.h,
      opacity: op,
      transform: `translateX(${(1 - dockIn) * 42}px)`,
      boxSizing: 'border-box',
      background: APP_SURFACE,
      borderLeft: `1px solid ${APP_LINE}`,
      boxShadow: 'inset 1px 0 0 rgba(45,38,32,0.035)',
      pointerEvents: 'none',
      zIndex: 34 }}>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 1,
        background: APP_LINE_STRONG, opacity: 0.78 }} />

      <div style={{ position: 'absolute', left: 22, right: 22, top: 20, height: 52,
        display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center',
        borderBottom: `1px solid ${APP_LINE}` }}>
        <div>
          <div style={appTyped({ fontSize: 15, fontWeight: 820, color: APP_TEXT })}>
            Kommentarverlauf
          </div>
          <div style={{ marginTop: 5, display: 'flex', gap: 14 }}>
            {['Chat', 'Referenzen'].map((tab, i) => (
              <span key={tab} style={appTyped({ fontSize: 10.2, fontWeight: 780,
                color: i === 0 ? APP_TEXT : APP_FAINT,
      ...
      <div style={{ position: 'absolute',
        left: OFFER_EDIT_PROMPT_BAR.x - OFFER_EDIT_SIDEBAR.x,
        top: OFFER_EDIT_PROMPT_BAR.y - OFFER_EDIT_SIDEBAR.y,
        width: OFFER_EDIT_PROMPT_BAR.w,
        height: OFFER_EDIT_PROMPT_BAR.h,
        borderRadius: 18, display: 'grid',
        gridTemplateColumns: '42px 1fr', alignItems: 'center', gap: 12,
        padding: '0 16px', boxSizing: 'border-box',
        opacity: promptBarIn,
        transform: `translateY(${(1 - promptBarIn) * 10}px) scale(${0.985 + promptBarIn * 0.015})`,
        background: promptSelected ? '#fff8ef' : APP_RAISED,
        border: `1px solid ${promptSelected ? 'rgba(240,168,94,0.56)' : APP_LINE_STRONG}`,
        boxShadow: promptSelected
          ? '0 18px 42px rgba(240,168,94,0.16), inset 0 1px 0 rgba(255,255,255,0.76)'
          : 'inset 0 1px 0 rgba(255,255,255,0.70)' }}>
        <span style={{ width: 36, height: 36, borderRadius: 13,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: 'rgba(240,168,94,0.13)', border: '1px solid rgba(240,168,94,0.28)' }}>
          <Icon name="sparkles" size={17} color={WARM} sw={2} />
        </span>
        <span style={{ minWidth: 0 }}>
          <div style={appTyped({ fontSize: 9.7, fontWeight: 840, color: APP_FAINT,
            letterSpacing: '0.12em', textTransform: 'uppercase' })}>
            Prompt Entwurf
          </div>
          <div style={{ marginTop: 6, ...appTyped({ fontSize: 13.6, lineHeight: 1.15,
            fontWeight: 780, color: APP_TEXT, whiteSpace: 'nowrap',
            overflow: 'hidden', textOverflow: 'ellipsis' }) }}>
            {promptText}
          </div>
        </span>
      ...
      <div style={{ position: 'absolute', left: 22, right: 22, top: 182,
        display: 'grid', gap: 12 }}>
        {commentThreads.map((item, i) => (
          <div key={item.author} style={{ borderRadius: 17, padding: '14px 14px',
            boxSizing: 'border-box',
            background: i === 0 ? '#ffffff' : APP_RAISED,
            border: `1px solid ${APP_LINE}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 9, height: 9, borderRadius: 99, background: item.color,
                boxShadow: `0 0 10px ${item.color}66` }} />
              <span style={appTyped({ fontSize: 12.3, fontWeight: 820, color: APP_TEXT })}>
                {item.author}
              </span>
              <span style={{ marginLeft: 'auto', ...appTyped({ fontSize: 9.2, fontWeight: 760,
                color: APP_FAINT, letterSpacing: '0.08em', textTransform: 'uppercase' }) }}>
            ...
            <div style={{ marginTop: 9, ...appTyped({ fontSize: 12.7, lineHeight: 1.35,
              fontWeight: 560, color: APP_MUTED }) }}>
              {item.text}
            </div>
          </div>
      ...
      <div style={{ position: 'absolute', left: 22, right: 22, top: 420 }}>
        <div style={appTyped({ fontSize: 10.4, fontWeight: 820, color: APP_FAINT,
          letterSpacing: '0.12em', textTransform: 'uppercase' })}>
          Verknüpfte Referenzen
        </div>
        <div style={{ marginTop: 10, display: 'grid', gap: 9 }}>
          {referenceItems.map((item) => (
            <div key={item.label} style={{ height: 46, borderRadius: 14,
              display: 'grid', gridTemplateColumns: '34px 1fr auto', alignItems: 'center',
              gap: 10, padding: '0 12px', boxSizing: 'border-box',
              background: APP_RAISED, border: `1px solid ${APP_LINE}` }}>
              <span style={{ width: 32, height: 32, borderRadius: 11,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: `${item.color}16`, border: `1px solid ${item.color}3d` }}>
                <Icon name={item.icon} size={16} color={item.color} sw={1.85} />
              </span>
              <span style={appTyped({ fontSize: 12.5, fontWeight: 780, color: APP_TEXT,
                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' })}>
                {item.label}
              </span>
              <span style={appTyped({ fontSize: 10.2, fontWeight: 760, color: APP_FAINT,
                whiteSpace: 'nowrap' })}>
                {item.meta}
              </span>
            </div>
      ...
      <div style={{ position: 'absolute', left: 22, right: 22, bottom: 20,
        height: 48, borderRadius: 999, display: 'grid', gridTemplateColumns: '1fr 76px',
        alignItems: 'center', gap: 8, padding: '0 8px 0 16px',
        boxSizing: 'border-box',
        background: '#ffffff', border: `1px solid ${APP_LINE_STRONG}` }}>
        <span style={appTyped({ fontSize: 12.2, fontWeight: 620, color: APP_FAINT })}>
          Änderung oder Frage eingeben...
        </span>
        <span style={{ height: 34, borderRadius: 999, display: 'flex',
          alignItems: 'center', justifyContent: 'center',
          background: APP_TEXT, border: `1px solid ${APP_TEXT}` }}>
          <span style={appTyped({ fontSize: 11.8, fontWeight: 820, color: APP_SURFACE })}>
            Senden
          </span>
        </span>
      </div>
    ...
    <div style={{ position: 'absolute', inset: 0, opacity: op, pointerEvents: 'none', zIndex: 28 }}>
      ...
      <svg width="1920" height="1080" viewBox="0 0 1920 1080"
        ...
        <path d="M 850 522 L 942 522 L 982 392"
          fill="none" stroke={APP_LINE_STRONG} strokeWidth="10" strokeLinecap="round"
          opacity={flow * 0.52} />
        <path d="M 850 522 L 942 522 L 982 392"
      ...
      <div style={{ position: 'absolute', left: 128, top: 248, width: 760, height: 610,
        ...
        boxShadow: '0 30px 86px rgba(45,38,32,0.14), 0 0 30px rgba(143,191,216,0.10), inset 0 1px 0 rgba(255,255,255,0.74)' }}>
        <div style={{ position: 'absolute', inset: 0,
          background: 'transparent' }} />
        ...
        <div style={{ position: 'absolute', left: 46, top: 108, right: 46 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 62, height: 62, borderRadius: 19, display: 'flex',
              alignItems: 'center', justifyContent: 'center',
              background: 'rgba(240,168,94,0.15)', border: '1px solid rgba(240,168,94,0.36)',
              boxShadow: '0 14px 34px rgba(45,38,32,0.12), 0 0 20px rgba(240,168,94,0.12)' }}>
          ...
          <div style={{ position: 'absolute', right: 0, top: -4, width: 186,
            display: 'grid', gap: 10 }}>
            {quickLinks.map((link, i) => {
              const qp = rise(l, 0.82 + i * 0.16, 0.42) * left;
              const pulse = link.click;
              return (
                <div key={link.label} style={{ height: 48, borderRadius: 16,
                  display: 'grid', gridTemplateColumns: '36px 1fr 24px', alignItems: 'center',
                  gap: 9, padding: '0 12px', boxSizing: 'border-box',
                  opacity: qp, transform: `translateY(${(1 - qp) * 8}px) scale(${1 - pulse * 0.025})`,
                  background: APP_RAISED,
                  border: `1px solid ${pulse > 0.01 ? `${link.color}9a` : APP_LINE}`,
                  boxShadow: pulse > 0.01
                    ? `0 0 ${18 + pulse * 20}px ${link.color}42, inset 0 1px 0 rgba(255,255,255,0.72)`
                    : 'inset 0 1px 0 rgba(255,255,255,0.66)' }}>
                  <span style={{ width: 32, height: 32, borderRadius: 11,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: `${link.color}18`, border: `1px solid ${link.color}4a` }}>
                    {link.icon === 'github' ? (
                      <img src="assets/tech-icons/github.svg" alt="" style={{ width: 18, height: 18, opacity: 0.72 }} />
                    ) : (
                      <img src="assets/tech-icons/microsoft-powerpoint.svg" alt="" style={{ width: 18, height: 18, opacity: 0.86 }} />
                    )}
                  </span>
                  <span style={{ fontFamily: MONO, fontSize: 11.5, letterSpacing: '0.10em',
                    textTransform: 'uppercase', color: APP_TEXT }}>{link.label}</span>
                  <Icon name="arrowUR" size={18} color={link.color} sw={1.85} />
                </div>
              );
            })}
          </div>
        </div>
        <div style={{ position: 'absolute', left: 46, right: 46, top: 202, bottom: 44 }}>
          <div style={{ position: 'absolute', left: 0, top: 0, width: 456, display: 'grid', gap: 12 }}>
            {meetingRows.map((row, i) => {
              const rp = rise(l, 0.46 + i * 0.10, 0.58) * left;
              return (
                <div key={`meeting-row-${i}`} style={{ height: 64, borderRadius: 19,
                  display: 'grid', gridTemplateColumns: '44px 1fr 102px', alignItems: 'center',
                  gap: 13, padding: '0 16px', boxSizing: 'border-box',
                  opacity: rp, transform: `translateY(${(1 - rp) * 7}px)`,
                  background: APP_RAISED,
                  border: `1px solid ${APP_LINE}`,
                  boxShadow: `inset 3px 0 0 ${row.color}52, 0 0 ${10 + rp * 10}px ${row.color}10` }}>
                  <span style={{ width: 34, height: 34, borderRadius: 12, display: 'flex',
                    alignItems: 'center', justifyContent: 'center',
                    background: `${row.color}18`, border: `1px solid ${row.color}4d`,
                    boxShadow: `0 0 16px ${row.color}18` }}>
                    <Icon name={row.icon} size={17} color={row.color} sw={1.85} />
                  </span>
                  <span style={{ display: 'grid', gap: 9 }}>
                    <span style={{ width: `${row.w1 * 100}%`, height: 8, borderRadius: 99,
                      background: APP_LINE_STRONG }} />
                    <span style={{ width: `${row.w2 * 100}%`, height: 6, borderRadius: 99,
                      background: APP_LINE }} />
                  </span>
                  <span style={{ height: 36, borderRadius: 999, display: 'flex',
                    alignItems: 'center', gap: 7, padding: '0 10px', boxSizing: 'border-box',
                    background: `${row.color}18`, border: `1px solid ${row.color}50`,
                    boxShadow: `0 0 ${8 + rp * 10}px ${row.color}18` }}>
                    {[0, 1, 2].map((dot) => (
                      <span key={dot} style={{ width: 10, height: 10, borderRadius: 99,
                        background: ['#74c69d', WARM, '#e8655a'][dot],
                        opacity: dot === row.active ? 0.96 : 0.24,
                        boxShadow: dot === row.active ? `0 0 12px ${row.color}88` : 'none' }} />
                    ))}
                    <span style={{ flex: 1, height: 5, borderRadius: 99,
                      background: row.color, opacity: row.answer }} />
                  </span>
                </div>
              );
            })}
          </div>
          <div style={{ position: 'absolute', right: 0, top: 2, width: 184, height: 258,
            borderRadius: 24, padding: 16, boxSizing: 'border-box',
            background: APP_RAISED,
            border: `1px solid ${APP_LINE}`,
            boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.64)' }}>
            <svg width="152" height="226" viewBox="0 0 152 226" style={{ position: 'absolute', left: 16, top: 16, overflow: 'visible' }}>
              <path d="M 76 18 L 76 208" fill="none" stroke={APP_LINE_STRONG} strokeWidth="2" />
              {meetingSources.map((source, i) => {
                const y = 22 + i * 60;
                const side = i % 2 === 0 ? -1 : 1;
                const p = rise(l, 0.66 + i * 0.12, 0.52) * left;
                const railX = 76;
                const cardEdgeX = side < 0 ? 57 : 97;
                return (
                  <g key={`meeting-source-${i}`} opacity={p}>
                    <path d={`M ${railX} ${y + 16} L ${cardEdgeX} ${y + 16}`}
                      stroke={source.color} strokeWidth="2.3" strokeLinecap="round"
                      opacity="0.58" />
                    <circle cx={railX} cy={y + 16} r="4.8" fill={source.color}
            ...
            {meetingSources.map((source, i) => {
              const p = rise(l, 0.72 + i * 0.12, 0.48) * left;
              const side = i % 2 === 0 ? 0 : 96;
              return (
                <div key={`meeting-source-card-${i}`} style={{ position: 'absolute',
                  left: 17 + side, top: 22 + i * 60, width: 56, height: 42,
                  borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  opacity: p, transform: `scale(${0.94 + p * 0.06})`,
                  background: `${source.color}16`, border: `1px solid ${source.color}48`,
                  boxShadow: `0 12px 30px rgba(45,38,32,0.10), 0 0 16px ${source.color}20` }}>
                  <Icon name={source.icon} size={20} color={source.color} sw={1.8} />
                </div>
              );
            })}
          </div>
          <div style={{ position: 'absolute', left: 4, right: 0, bottom: 0, height: 50 }}>
            <span style={{ position: 'absolute', left: 0, right: 0, top: 20, height: 2,
              borderRadius: 99, background: APP_LINE_STRONG }} />
            {[0, 1, 2, 3, 4].map((i) => {
              const color = ['#8fbfd8', WARM, '#e8655a', '#c65bb0', '#74c69d'][i];
              const x = 20 + i * 139;
              const p = rise(l, 1.02 + i * 0.08, 0.46) * left;
              return (
                <span key={`meeting-time-${i}`} style={{ position: 'absolute',
                  left: x, top: 12, width: 17, height: 17, borderRadius: 99,
                  opacity: p, transform: `scale(${0.82 + p * 0.18})`,
                  background: color,
                  boxShadow: `0 0 14px ${color}72` }} />
              );
            })}
          </div>
        </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: op, pointerEvents: 'none', zIndex: 62 }}>
      <div style={{ position: 'absolute', left: 982, top: 164, width: 798, height: 300,
        opacity: githubOpen,
        transform: `translateY(${(1 - githubOpen) * 18}px) scale(${0.975 + githubOpen * 0.025})`,
        transformOrigin: '35% 18%',
        borderRadius: 22, overflow: 'hidden',
        background: '#0d1117',
        border: '1px solid rgba(143,191,216,0.24)',
        boxShadow: '0 38px 110px rgba(0,0,0,0.44), 0 0 34px rgba(143,191,216,0.14)' }}>
      ...
      <div style={{ position: 'absolute', left: 1008, top: 512, width: 804, height: 318,
        opacity: deckOpen,
        transform: `translateY(${(1 - deckOpen) * 24}px) scale(${0.972 + deckOpen * 0.028})`,
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 54,
          ...
          <div style={{ position: 'absolute', left: 18, top: 11, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontFamily: DISPLAY, fontWeight: 790, fontSize: 24,
              letterSpacing: '-0.018em', color: APP_TEXT }}>Konzept-Präsentation</span>
          </div>
...
function PersistentPromptAgentBridge() {
  ...
  const end = SCENE_MATCHED_START + 0.36;         // keep the completed agent message landed before handoff
...
const FINANCE_CHART_DRAW_DUR = 4.50 * (FINANCE_CHART_DRAW_MAX / 50);
...
function SceneFinanz(local) {
  ...
  const renderChart = (cfg) => {
    ...
    const markerDock = Easing.easeInOutCubic(clamp((q - 0.56) / 0.44, 0, 1));
    const markerW = 252;
    const markerH = 48;
    const markerFollowQ = Math.min(q, 0.88);
...
const LOGO_BRIDGE_START = TEAM_MATCHED_CONSULTANTS_DONE + 0.40;
...
const LOGO_BRIDGE_OFFER_PAYOFF_DONE = LOGO_BRIDGE_OFFER_START + 3.18;
const LOGO_BRIDGE_DEAL_PAUSE = 0.72;
...
const LOGO_BRIDGE_END = LOGO_BRIDGE_PROJECT_MORPH_START + 1.48;
...
function OfferOutreachAppScene() {
  const t = useTime();
  const start = LOGO_BRIDGE_OFFER_LABELS_START - 0.28;
  const end = LOGO_BRIDGE_DEAL_START + 0.18;
  const op = band(t, start, end, 0.34);
  if (op <= 0.001) return null;

  const local = t - start;
  const exit = rise(t, LOGO_BRIDGE_DEAL_START - 0.22, 0.40);
  ...
  const deliverables = [
    {
      title: 'CVs',
      detail: 'Kundenprofile, Projektreferenzen, Verfuegbarkeit',
      icon: 'users',
      meta: '3 Profile',
      status: 'bereit',
      target: 96,
      refs: ['Profil', 'Projekt', 'Skills'],
      rows: ['Max', 'Lena', 'Jonas'],
    },
    {
      title: 'Dienstleistung & Vertragsentwurf',
      detail: 'Scope, Rollen, Aufwand, Vertragsbausteine',
      icon: 'file',
      meta: 'Angebot',
      status: 'in Review',
      target: 91,
      refs: ['Scope', 'Aufwand', 'Vertrag'],
      rows: ['Leistungsumfang', 'PT / Laufzeit', 'Anlagen'],
    },
  ];
  const consultants = [
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: panelOpacity,
      ...
      <div style={{ position: 'absolute', left: 228, top: 164, width: 1468, height: 794,
        boxSizing: 'border-box',
        background: 'transparent',
        border: 'none',
        boxShadow: 'none',
        overflow: 'visible' }}>
        <div style={{ position: 'absolute', left: 34, top: 34, width: 558, height: 704,
          background: 'transparent', border: 'none', boxShadow: 'none',
          padding: 0, boxSizing: 'border-box' }}>
          <div style={{ display: 'grid', gap: 18 }}>
            {consultants.map((person, i) => {
              const row = rise(local, 0.28 + i * 0.12, 0.42);
              return (
                <div key={person.name} style={{ position: 'relative', height: 176, borderRadius: 20,
                  background: APP_SURFACE, border: `1px solid ${APP_LINE_STRONG}`,
                  display: 'grid', gridTemplateColumns: '78px 1fr 70px', alignItems: 'center',
                  gap: 18, padding: '0 22px', boxSizing: 'border-box',
                  boxShadow: '0 16px 38px rgba(45,38,32,0.08)',
                  opacity: row, transform: `translateX(${(1 - row) * -22}px)` }}>
                  <span style={{ position: 'absolute', left: 0, top: 20, bottom: 20,
                    width: 3, borderRadius: '0 99px 99px 0', background: rowAccentSoft }} />
                  <ConsultantAvatar person={person} size={66} selected={row} />
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontFamily: DISPLAY, fontWeight: 840, fontSize: 24,
                        color: APP_TEXT, letterSpacing: '-0.018em', whiteSpace: 'nowrap' }}>
                        {person.name}
                      </span>
                      <span style={{ height: 27, minWidth: 50, borderRadius: 999,
                        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                        background: APP_RAISED, border: `1px solid ${APP_LINE}`,
                        ...appTyped({ fontSize: 13, fontWeight: 820, color: APP_MUTED }) }}>
                        {person.score}
                      </span>
                    </div>
                    <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span style={{ ...appTyped({ fontSize: 14, fontWeight: 740, color: APP_MUTED }) }}>{person.role}</span>
                      {person.certs.slice(0, 2).map((cert) => (
                        <CertificationBadge key={cert} cert={cert} color={person.color} mini />
                      ))}
                    </div>
                    <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 64px', gap: 12, alignItems: 'center' }}>
                      <div style={{ height: 7, borderRadius: 99, background: APP_RAISED, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${Math.round(mix(18, person.availability * 100, row))}%`,
                          background: person.color, borderRadius: 99 }} />
                      </div>
                      <span style={{ ...appTyped({ fontSize: 12, fontWeight: 780, color: APP_FAINT }) }}>Kapazität</span>
                    </div>
                  </div>
                  <div style={{ justifySelf: 'end', width: 46, height: 46, borderRadius: 14,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: APP_RAISED,
                    border: `1px solid ${APP_LINE_STRONG}` }}>
                    <Icon name={i === 0 ? 'check' : 'arrowUR'} size={19} color={rowAccent} sw={2.3} />
                  </div>
                </div>
              );
            })}
          </div>
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 112,
            borderRadius: 20, background: APP_RAISED, border: `1px solid ${APP_LINE}`,
            display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center',
            padding: '0 22px', boxSizing: 'border-box' }}>
            <div>
              <div style={{ ...appTyped({ fontSize: 12, fontWeight: 840, color: APP_MUTED,
                textTransform: 'uppercase', letterSpacing: '0.12em' }) }}>
                Quellenstand
              </div>
              <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                {['Projektkontext', 'Skills', 'Compliance'].map((label, i) => (
                  <span key={label} style={{ height: 28, borderRadius: 999, display: 'inline-flex',
                    alignItems: 'center', gap: 7, padding: '0 10px',
                    background: APP_SURFACE, border: `1px solid ${APP_LINE_STRONG}` }}>
                    <span style={{ width: 7, height: 7, borderRadius: 99,
                      background: i === 0 ? '#8fbfd8' : i === 1 ? rowAccent : '#c65bb0' }} />
                    <span style={{ ...appTyped({ fontSize: 11, fontWeight: 780, color: APP_MUTED }) }}>{label}</span>
                  </span>
                ))}
              </div>
            </div>
            <span style={{ height: 36, borderRadius: 999, display: 'inline-flex',
              alignItems: 'center', padding: '0 13px',
              background: APP_SURFACE, border: `1px solid ${APP_LINE_STRONG}` }}>
              <span style={{ ...appTyped({ fontSize: 13, fontWeight: 840, color: APP_TEXT }) }}>3 aktiv</span>
            </span>
          </div>
        </div>

        <div style={{ position: 'absolute', left: 628, top: 26, width: 826, height: 724,
          background: 'transparent', border: 'none', boxShadow: 'none',
          padding: 0, boxSizing: 'border-box' }}>
          <div style={{ display: 'grid', gap: 24 }}>
            {deliverables.map((item, i) => {
              const p = rise(local, 0.52 + i * 0.20, 0.50);
              const gen = rise(local, 1.02 + i * 0.20, 0.76);
              const progress = Math.round(mix(28, item.target, gen));
              return (
                <div key={item.title} style={{ position: 'relative', height: 344, borderRadius: 22,
                  background: APP_SURFACE, border: `1px solid ${APP_LINE_STRONG}`,
                  boxShadow: '0 18px 44px rgba(45,38,32,0.08)',
                  opacity: p, transform: `translateX(${(1 - p) * 22}px)`,
                  padding: '24px 28px', boxSizing: 'border-box' }}>
                  <span style={{ position: 'absolute', left: 0, top: 24, bottom: 24,
                    width: 3, borderRadius: '0 99px 99px 0', background: rowAccentSoft }} />
                  <div style={{ display: 'grid', gridTemplateColumns: '72px 1fr 132px', gap: 20, alignItems: 'start' }}>
                    <div style={{ width: 64, height: 64, borderRadius: 18, display: 'flex',
                      alignItems: 'center', justifyContent: 'center',
                      background: rowTint, border: `1px solid ${APP_LINE}` }}>
                      <Icon name={item.icon} size={31} color={rowAccent} sw={2.1} />
                    </div>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
                        <span style={{ fontFamily: DISPLAY, fontWeight: 850, fontSize: i === 0 ? 34 : 29,
                          color: APP_TEXT, letterSpacing: '-0.022em', lineHeight: 1.02,
                          whiteSpace: i === 0 ? 'nowrap' : 'normal' }}>
                          {item.title}
                        </span>
                        <span style={{ height: 29, borderRadius: 999, display: 'inline-flex',
                          alignItems: 'center', padding: '0 11px', flexShrink: 0,
                          background: APP_RAISED, border: `1px solid ${APP_LINE}`,
                          ...appTyped({ fontSize: 12, fontWeight: 820, color: APP_MUTED }) }}>
                          {item.meta}
                        </span>
                      </div>
                      <div style={{ marginTop: 10, ...appTyped({ fontSize: 15, fontWeight: 740,
                        color: APP_MUTED, lineHeight: 1.32 }) }}>
                        {item.detail}
                      </div>
                    </div>
                    <div style={{ justifySelf: 'end', display: 'grid', gap: 10, justifyItems: 'end' }}>
                      <span style={{ height: 34, minWidth: 112, borderRadius: 999,
                        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                        background: gen > 0.88 ? APP_TEXT : APP_RAISED,
                        border: `1px solid ${APP_LINE_STRONG}`,
                        ...appTyped({ fontSize: 12, fontWeight: 860,
                          color: gen > 0.88 ? APP_SURFACE : APP_MUTED }) }}>
                        {item.status}
                      </span>
                      <span style={{ width: 112, height: 40, borderRadius: 13,
                        display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                        background: APP_RAISED, border: `1px solid ${APP_LINE_STRONG}` }}>
                        <Icon name="arrowUR" size={15} color={APP_MUTED} sw={2.2} />
                        <span style={{ ...appTyped({ fontSize: 13, fontWeight: 800, color: APP_TEXT }) }}>
                          öffnen
                        </span>
                      </span>
                    </div>
                  </div>
                  <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 64px',
                    alignItems: 'center', gap: 18 }}>
                    <div style={{ height: 8, borderRadius: 99, background: APP_RAISED, overflow: 'hidden' }}>
                  ...
                  <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 14 }}>
                    <div style={{ minHeight: 108, borderRadius: 16, background: APP_RAISED,
                      border: `1px solid ${APP_LINE}`, padding: '13px 15px', boxSizing: 'border-box' }}>
                      <div style={{ ...appTyped({ fontSize: 11, fontWeight: 840, color: APP_FAINT,
                        textTransform: 'uppercase', letterSpacing: '0.12em' }) }}>
                        Workflow
                      </div>
                      <div style={{ marginTop: 11, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 9 }}>
                        {item.rows.slice(0, 3).map((label, idx) => (
                          <span key={`${item.title}-step-${label}`} style={{ minHeight: 52, borderRadius: 14,
                            display: 'grid', alignContent: 'center', justifyItems: 'start', gap: 7,
                            padding: '0 10px', background: APP_SURFACE,
                            border: `1px solid ${APP_LINE_STRONG}` }}>
                            <span style={{ width: 20, height: 20, borderRadius: 7,
                              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                              background: idx < 2 ? APP_TEXT : APP_RAISED,
                              border: `1px solid ${APP_LINE_STRONG}` }}>
                              {idx < 2 && <Icon name="check" size={12} color={APP_SURFACE} sw={2.3} />}
                            </span>
                            <span style={{ ...appTyped({ fontSize: 10.5, fontWeight: 780, color: APP_MUTED,
                    ...
                    <div style={{ minHeight: 108, borderRadius: 16, background: APP_RAISED,
                      ...
                      <div style={{ marginTop: 11, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {item.refs.map((ref, idx) => (
                          <span key={`${item.title}-${ref}`} style={{ height: 30, borderRadius: 999,
                            display: 'inline-flex', alignItems: 'center', gap: 7,
                            padding: '0 10px', background: APP_SURFACE,
                            border: `1px solid ${APP_LINE_STRONG}` }}>
                            <span style={{ width: 7, height: 7, borderRadius: 99,
                              background: idx === 0 ? APP_TEXT : rowAccentSoft }} />
                            <span style={{ ...appTyped({ fontSize: 11, fontWeight: 780, color: APP_MUTED }) }}>
                              {ref}
                            </span>
                          </span>
                        ))}
                      ...
                      <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 7 }}>
                        {[0, 1, 2].map((dot) => (
                          <span key={dot} style={{ width: 9, height: 9, borderRadius: 99,
                            background: dot <= i + 1 ? rowAccent : APP_LINE_STRONG }} />
                        ))}
                        <span style={{ marginLeft: 2, ...appTyped({ fontSize: 11, fontWeight: 760, color: APP_FAINT }) }}>
                          verknüpft
                        </span>
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: 14,
                    ...
                    <span style={{ ...appTyped({ fontSize: 12, fontWeight: 760, color: APP_FAINT }) }}>
                      Version 0{i + 2} · letzte Änderung gerade eben
                    </span>
                    <span style={{ ...appTyped({ fontSize: 12, fontWeight: 820, color: APP_MUTED }) }}>
                      {i === 0 ? 'kundenbereit' : 'Review läuft'}
                    </span>
                  </div>
                </div>
              );
